...
feel free to reach out to us on the [TMAP8 discussion page](https://github.com/idaholab/TMAP8/discussions).
...
The next step is to convert these two strong-form PDEs into their weak forms by multiplying with a test function, $\psi$, and integrating over a domain, $\Omega$ with surface $\delta \Omega$. Using a divergence theorem, one can obtain the weak form of the mass conservation equation ([eq:concentration]) as follows:
...
Similarly, the weak form of the conservation of energy equation can be written by multiplying [eq:temperature] with a test function, $\psi$, and integrating over a domain, $\Omega$. Using the divergence theorem, one can obtain the weak form of the conservation of energy equation as follows:
...
For the mass conservation equation, we use `ADTimeDerivative` and `ADMatDiffusion` to solve the 1$^{st}$ and 3$^{rd}$ terms of [eq:concentration_weak].
The TMAP8 kernels `TrappingNodalKernel` and `ReleasingNodalKernel` solve the 4$^{th}$ and 5$^{th}$ terms of [eq:concentration_weak],
...
For the mass conservation equation, we use `FunctionNeumannBC` and `DirichletBC` to solve the 2$^{nd}$ term of [eq:concentration_weak].
...
For the energy conservation equation, we use `FunctionNeumannBC` and `DirichletBC` to solve the 2$^{nd}$ term of [eq:temperature_weak].
