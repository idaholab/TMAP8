...
from matplotlib.colors import TABLEAU_COLORS as plt_colors
import pandas as pd
...
def json_to_RESULTS(json_path=None, post_dictionary=None, **kwargs):
    ...
    json_results.data_frame = (
        json_dict["data_frame"] if "data_frame" in json_dict.keys() else None
    )
    json_results.data_vectors = (
        json_dict["data_vectors"] if "data_vectors" in json_dict.keys() else {}
    )
    for vector, results in json_results.data_vectors.items():
        json_results.data_vectors[vector] = np.asarray(results)
    json_results.post_processing_vectors = (
        json_dict["post_processing_vectors"]
        if "post_processing_vectors" in json_dict.keys()
        else {}
    )
    for post_vector, step_dict in json_results.post_processing_vectors.items():
        ...
        for step, result_object in step_dict.items():
            json_results.post_processing_vectors[post_vector][step] = json_to_RESULTS(
                post_dictionary=result_object
            )
    return json_results
...
class RESULTS:
    ...
    def read_vector_post_processor_data(
        self,
        directory_path: str,
        post_processing_vector: str,
        *args,
        ...
        **kwargs,
        ...
        for file in os.listdir(directory_path):
            if post_processing_vector in file:
                vector_post_processing_outputs.append(file)
        ...
        for file in vector_post_processing_outputs:
            self.post_processing_vectors[post_processing_vector][
                (file[:-4].split("_"))[-1]
            ] = RESULTS(os.path.join(directory_path, file))
            for vector in vector_list:
                self.post_processing_vectors[post_processing_vector][
                    (file[:-4].split("_"))[-1]
                ].read_vector(vector)
    ...
    def save_vector(
        ...
        *args,
        path: str = "",
        vector: str = None,
        post_processor_vector: str = None,
        step: str = None,
        suppress_json: bool = False,
        **kwargs,
    ) -> Union[None, dict]:
        if post_processor_vector:
            saved_vector = self.post_processing_vectors[post_processor_vector][
                step
            ].data_vectors[vector]
        else:
        ...
        if suppress_json:
            ...
            with open(f"{path}.json", "w+") as ftw:
                ftw.write(
                    json.dumps(
                        {"name": vector, "data": saved_vector.tolist()}, indent=4
                    )
                )
        return ()
    ...
    def save_vectors(
        ...
        path: str,
        *args,
        vector_list: list[str] = None,
        post_processor_vector_list: list[str] = [],
        step_list: list[str] = [],
        **kwargs,
    ) -> None:
        saved_dict = {}
            ...
            if post_processor_vector_list:
                for post_vector in post_processor_vector_list:
                    for step in step_list:
                        saved_dict[f"{post_vector}_{step}_{vector}"] = self.save_vector(
                            vector=vector,
    ...
    def save_object(
        ...
        path: str,
        *args,
        do_not_print: list[str] = ["data_frame", "plots"],
        suppress_save=False,
        **kwargs,
    ) -> str:
        ...
        for vector, step_dict in self.post_processing_vectors.items():
            ...
            for step, result in step_dict.items():
                post_processing_vectors_dict[vector][step] = result.save_object(
                    path="", *args, do_not_print=do_not_print, suppress_save=True
                )
    ...
    def get_scaled_vector(
        self, vector: str, factor: float, *args, new_name: str = "", **kwargs
    ) -> ndarray:
    ...
    def get_normalized_vector(
        self, vector: str, *args, by_total: bool = False, new_name: str = "", **kwargs
    ) -> ndarray:
        un_normalized = self.data_vectors[vector]
        norm_factor = (
            sum(un_normalized.tolist()) if by_total else max(un_normalized.tolist())
        )
    ...
    def get_cumulative_function(
        self, vector: str, *args, new_name: str = "", **kwargs
    ) -> ndarray:
    ...
    def get_vector_dot_product(
        ...
        **kwargs,
        ...
        dot_product = (
            np.dot(func_1, func_2) if not element_wise else np.multiply(func_1, func_2)
        )
    ...
    def sum_vectors(
        self, vector_list: list[str], *args, new_name: str = "", **kwargs
    ) -> ndarray:
        if new_name:
            new_vector = new_name
        else:
    ...
    def get_closest_point(
        ...
        vector: str,
        *args,
        corresponding_vectors: list[str] = [],
        **kwargs,
        ...
        closest = {
            ...
            "corresponding_values": [
                float(self.data_vectors[i][closest_index])
                for i in corresponding_vectors
            ],
        }
        return closest

    def manipulate_post_processor_data(
        self, post_processor_vector, *args, function=None, **kwargs
    ):
        # active_function = function
        for time_step, step_results in self.post_processing_vectors[
    ...
    def apply_kernel(
        ...
        **kwargs,
        ...
        if not post_processing_vector:
            ...
            result = function(
                self.post_processing_vectors[post_processing_vector][step].data_vectors[
                    vector
                ]
            )
            self.post_processing_vectors[post_processing_vector][step].data_vectors[
    ...
    def plot_time_series_vector(
        ...
        **kwargs,
        ...
        for time_step, step_result in self.post_processing_vectors[
            post_processing_vector
        ].items():
            if time_step != "0000":
                ax.set_title(
                    f"{title} at {np.round(self.data_vectors['time'][int(time_step)], 1)}s"
                )
                ...
                ax.plot(
                    xvector, yvector, label=time_step, color="darkblue", linewidth=1.5
                )
    ...
    def plot_time_series_vectors(
        ...
        **kwargs,
        ...
        titles = (
            titles
            if titles
            else [f"{xlabel} vs {ylabels[i]}" for i in range(0, len(y_list))]
        )
        plt.ion()
        ...
        for time_step, step_result in self.post_processing_vectors[
            ...
            if time_step != "0000":
                ...
                for y in y_list:
                    ax[i].set_title(
                        f"{titles[i]} at {np.round(self.data_vectors['time'][int(time_step)], 1)}s"
                    )
                    yvector = step_result.data_vectors[y]
    ...
    def general_plot(
        ...
        x_list = x_list if len(x_list) > 1 else [x_list[0] for i in y_list]
        labels = labels if labels else y_list
        labels = labels if len(labels) > 1 else [labels[0] for i in y_list]
        fig, ax = plt.subplots(1, 1, figsize=(6, 4))
        ...
        ax.set_facecolor("aliceblue")
        ax.grid(
            visible=True, which="both", axis="both", color="gainsboro", linestyle=":"
        )
        ...
        for i in range(0, len(y_list)):
            if not post_processor_vector:
                ax.plot(
                    self.data_vectors[x_list[i]],
                    self.data_vectors[y_list[i]],
                    label=labels[i],
                    linewidth=1.5,
                )
            else:
                ax.plot(
                    self.post_processing_vectors[post_processor_vector][
                        step
                    ].data_vectors[x_list[i]],
                    self.post_processing_vectors[post_processor_vector][
                        step
                    ].data_vectors[y_list[i]],
                    label=labels[i],
                    color="darkblue",
                    linewidth=1.5,
                )
    ...
    def general_plot2(
        ...
        **kwargs,
        ...
        x_vector = (
            self.data_vectors[x]
            if not post_processor_vector
            else self.post_processing_vectors[post_processor_vector][step].data_vectors[
                x
            ]
        )
        y_vector = (
        ...
        x_limits = [0, 0]
        x_limits[0] = x_min if x_min else min(x_vector)
        x_limits[1] = x_max if x_max else max(x_vector)
        y_limits = [0, 0]
        y_limits[0] = y_min if y_min else min(y_vector)
        y_limits[1] = y_max if y_max else max(y_vector)

        new_axes.xlabel = x_label if x_label else x
        plt.ylabel = y_label if y_label else y
        plt.xlim(x_limits)
        plt.ylim(y_limits)
        new_axes.xscale = x_scale
        plt.yscale = y_scale
        plt.plot(
            x_vector, y_vector, label=plot_label, marker=marker, linewidth=linewidth
        )
    ...
    def plot_vector(
        ...
        ylabel = ylabel if ylabel else y
        x_vector = (
            self.post_processing_vectors[post_processor_vector][step].data_vectors[x]
            if post_processor_vector
            else self.data_vectors[x]
        )
        y_vector = (
            self.post_processing_vectors[post_processor_vector][step].data_vectors[y]
            if post_processor_vector
            else self.data_vectors[y]
        )
        ...
        if x_max and x_min:
            ...
            print("huh")
        if x_max and not x_min:
            plt.xlim(right=x_max)
        if x_min and not x_max:
            plt.xlim(left=x_min)
        if y_max and y_min:
            plt.ylim((y_min, y_max))
        if y_max and not y_min:
            plt.ylim(top=y_max)
        if y_min and not y_max:
            plt.ylim(bottom=y_min)
        my_axs = plt.gca()
        plt.show()

    def plot_arbitrary_number(
        self,
        x,
        y_list,
        post_processing_vector=None,
        ...
        for y in y_list:
            y_vector = (
                self.post_processing_vectors[post_processing_vector][y][plot_variable]
                if post_processing_vector
                else self.data_vectors[y]
            )
    ...
    def plot_vector_pairs(
        ...
        for xy in xy_list:
            ...
            y = xy[1]
            x_vector = (
                self.post_processing_vectors[post_processing_vector][x].data_vectors[
                    plot_variable
                ]
                if post_processing_vector
                else self.data_vectors[x]
            ...
            y_vector = (
                self.post_processing_vectors[post_processing_vector][y].data_vectors[
                    plot_variable
                ]
                if post_processing_vector
                else self.data_vectors[y]
...
class COMPARISON:
    ...
    def get_shared_vector(self, vector):
        self.shared[vector] = (
            self.comp1.data_vectors[vector]
            if vector in self.comp1.data_vectors.keys()
            else self.comp2.data_vectors[vector]
        )
    ...
    def abs_diff(self, vector):
        ...
        self.comparisons[f"{vector}_abs_diff"] = np.subtract(
            self.comp1.data_vectors[vector], self.comp2.data_vectors[vector]
        )
    ...
    def rel_diff_2to1(self, vector):
        ...
        self.comparisons[f"{vector}_rel_diff_2to1"] = np.divide(
            np.subtract(
                self.comp2.data_vectors[vector], self.comp1.data_vectors[vector]
            ),
            self.comp1.data_vectors[vector],
        )
    ...
    def rel_diff_1to2(self, vector):
        ...
        self.comparisons[f"{vector}_rel_diff_1to2"] = np.divide(
            np.subtract(
                self.comp1.data_vectors[vector], self.comp2.data_vectors[vector]
            ),
            self.comp2.data_vectors[vector],
        )
    ...
    def general_plot(
        ...
        x_list_1 = x_list_1 if len(x_list_1) > 1 else [x_list_1[0] for i in y_list_1]
        labels_1 = labels_1 if labels_1 else y_list_1
        labels_1 = labels_1 if len(labels_1) > 1 else [labels_1[0] for i in y_list_1]
        x_list_2 = x_list_2 if len(x_list_2) > 1 else [x_list_2[0] for i in y_list_2]
        labels_2 = labels_2 if labels_2 else y_list_2
        labels_2 = labels_2 if len(labels_2) > 1 else [labels_2[0] for i in y_list_2]
        ...
        for i in range(0, len(y_list_1)):
            if not post_processor_vector:
                ax.plot(
                    self.comp1.data_vectors[x_list_1[i]],
                    self.comp1.data_vectors[y_list_1[i]],
                    label=labels_1[i],
                    linewidth=1.5,
                )
            else:
                ax.plot(
                    self.comp1.post_processing_vectors[post_processor_vector][
                        step
                    ].data_vectors[x_list_1[i]],
                    self.comp1.post_processing_vectors[post_processor_vector][
                        step
                    ].data_vectors[y_list_1[i]],
                    label=labels_1[i],
                    color="darkblue",
                    linewidth=1.5,
                )
        for i in range(0, len(y_list_2)):
            if not post_processor_vector:
                ax.plot(
                    self.comp2.data_vectors[x_list_2[i]],
                    self.comp2.data_vectors[y_list_2[i]],
                    label=labels_2[i],
                    linewidth=1.5,
                )
            else:
                ax.plot(
                    self.comp2.post_processing_vectors[post_processor_vector][
                        step
                    ].data_vectors[x_list_2[i]],
                    self.comp2.post_processing_vectors[post_processor_vector][
                        step
                    ].data_vectors[y_list_2[i]],
                    label=labels_2[i],
                    color="darkblue",
                    linewidth=1.5,
                )
...
class SOBOL:
    def __init__(self, path, inputs=[], time_step=0):
        ...
        if inputs == []:
            for key in self.reference_dict["time_steps"][self.time_step][
                "matrix"
            ].keys():
                if "results" not in key:
                    inputs.append(key)  # ; print(key)
            self.inputs = inputs

        self.sobol = {
            "confidence_intervals": self.reference_dict["reporters"]["sobol"][
                "confidence_intervals"
            ]["levels"],
            "number_of_samples": self.reference_dict["reporters"]["sobol"][
                "confidence_intervals"
            ]["replicates"],
            "inputs": inputs,
            ...
            "outputs": [
                (i.split(":"))[-2]
                for i in self.reference_dict["reporters"]["sobol"]["values"].keys()
            ],
            "number_of_outputs": len(
                self.reference_dict["reporters"]["sobol"]["values"].keys()
            ),
        }
        self.statistics = {
            "confidence_intervals": self.reference_dict["reporters"]["sobol"][
        ...
        for key in self.reference_dict["time_steps"][0]["stats"].keys():
            ...
            out = out[-2]
            if out not in stat_outs:
                stat_outs.append(out)
    ...
    def get_sobol_1st(self):
        ...
        for output in self.sobol["outputs"]:
            ...
            for inp in self.sobol["inputs"]:
                sobol_first[output]["mean"][inp] = self.reference_dict["time_steps"][
                    self.time_step
                ]["sobol"][f"results_results:{output}:value"]["FIRST_ORDER"][0][j]
                j += 1
            i = 0
            for key in keys:
                j = 0
                ...
                for inp in self.sobol["inputs"]:
                    sobol_first[output][key][inp] = self.reference_dict["time_steps"][
                        self.time_step
                    ]["sobol"][f"results_results:{output}:value"]["FIRST_ORDER"][1][i][
                        j
                    ]
                    j += 1
                i += 1
    ...
    def get_sobol_2nd(self):
        ...
        for output in self.sobol["outputs"]:
            ...
            for inp in self.inputs:
                sobol_scd[output]["mean"][inp] = self.reference_dict["time_steps"][
                    self.time_step
                ]["sobol"][f"results_results:{output}:value"]["SECOND_ORDER"][0][j]
                j += 1
            ...
            for key in keys:
                ...
                j = 0
                for inp in self.inputs:
                    sobol_scd[output][key][inp] = self.reference_dict["time_steps"][
                        self.time_step
                    ]["sobol"][f"results_results:{output}:value"]["SECOND_ORDER"][1][i][
                        j
                    ]
                    j += 1
                i += 1
    ...
    def get_sobol_total(self):
        ...
        for output in self.sobol["outputs"]:
            ...
            j = 0
            for inp in self.sobol["inputs"]:
                sobol_tot[output]["mean"][inp] = self.reference_dict["time_steps"][
                    self.time_step
                ]["sobol"][f"results_results:{output}:value"]["TOTAL"][0][j]
                j += 1
            i = 0
            for key in keys:
                j = 0
                ...
                for inp in self.sobol["inputs"]:
                    sobol_tot[output][key][inp] = self.reference_dict["time_steps"][
                        self.time_step
                    ]["sobol"][f"results_results:{output}:value"]["TOTAL"][1][i][j]
                    j += 1
                i += 1
    ...
    def get_var_accounted_for(self):
        ...
        for output in self.sobol["outputs"]:
            ...
            for col in self.reference_dict["time_steps"][0]["sobol"][
                f"results_results:{output}:value"
            ]["SECOND_ORDER"][0]:
                for row in col:
                    var_acc_for += row
    ...
    def get_statistics(self):
        ...
        for output in self.statistics["outputs"]:
            ...
            self.statistics["statistics"][output]["value"]["mean"] = (
                self.reference_dict["time_steps"][self.time_step]["stats"][
                    f"results_results:{output}:value_MEAN"
                ][0]
            )
            self.statistics["statistics"][output]["stddev"]["mean"] = (
                self.reference_dict["time_steps"][self.time_step]["stats"][
                    f"results_results:{output}:value_STDDEV"
                ][0]
            )
            i = 0
            for ci in self.statistics["confidence_intervals"]:
                self.statistics["statistics"][output]["value"][ci] = (
                    self.reference_dict["time_steps"][self.time_step]["stats"][
                        f"results_results:{output}:value_MEAN"
                    ][1][i]
                )
                self.statistics["statistics"][output]["stddev"][
                    ci
                ] = self.reference_dict["time_steps"][self.time_step]["stats"][
                    f"results_results:{output}:value_STDDEV"
                ][
        ...
        for output in self.statistics["outputs"]:
            self.statistics["statistics"][output] = pd.DataFrame(
                self.statistics["statistics"][output]
            )
    ...
    def get_matrix(self):
        ...
        for inp in inputs:
            try:
                inp_matrix[inp] = self.reference_dict["time_steps"][self.time_step][
                    f"matrix/{inp}"
                ]["value"]
            except:
                try:
        ...
        outp_matrix = {}
        for outp in outputs:
            outp_matrix[outp] = self.reference_dict["time_steps"][self.time_step][
                "matrix"
            ][f"matrix_results:{outp}:value"]

        # package as attribute
        self.matrix = {
            "inputs": pd.DataFrame(inp_matrix),
            "outputs": pd.DataFrame(outp_matrix),
        }
        return self.matrix
    ...
    def estimate_state_frequency(
        ...
        if use_matrix:
            print(
                f"Using a frequency approach based on <{self.statistics['number_of_samples']}> samples."
            )
            ...
            if method == "gamma":
                ...
                if pdf and plot:
                    ...
                    plt.plot(
                        np.linspace(min(data) - 50, max(data) + 50, num=len(data)),
                        scipy.gamma.pdf(
                            np.linspace(min(data) - 50, max(data) + 50, num=len(data)),
                            *curve_args,
                        ),
                    ...
                    self.plot_percentiles(
                        labels=[output], plot=[output], normalize=normalize, flip=True
                    )
                    plt.plot(
                        np.linspace(min(data), max(data), num=len(data)),
                        scipy.gamma.cdf(
                            np.linspace(min(data), max(data), num=len(data)),
                            *curve_args,
                        ),
                ...
                chi_squared, p_value = scipy.chisquare(
                    scipy.relfreq(data, numbins=int(0.01 * len(data)))[0],
                    scipy.relfreq(
                        scipy.gamma.pdf(
                            np.linspace(min(data), max(data), num=len(data)),
                            *curve_args,
                        ),
                        numbins=int(0.01 * len(data)),
                    )[0],
                    axis=0,
                )
                ...
                if pdf and plot:
                    ...
                    plt.plot(
                        np.linspace(min(data), max(data)),
                        scipy.lognorm.pdf(
                            np.linspace(min(data), max(data)), *curve_args
                        ),
                    )
                elif plot:
                    self.plot_percentiles(
                ...
                if pdf and plot:
                    ...
                    plt.plot(
                        np.linspace(min(data), max(data)),
                        scipy.beta.pdf(np.linspace(min(data), max(data)), *curve_args),
                    )
                elif plot:
                    self.plot_percentiles(
                        labels=[output], plot=[output], normalize=normalize, flip=True
                    ...
                    plt.plot(
                        np.linspace(min(data), max(data)),
                        scipy.beta.cdf(np.linspace(min(data), max(data)), *curve_args),
                    )
                ...
                if pdf and plot:
                    ...
                    plt.plot(
                        np.linspace(min(data), max(data)),
                        scipy.tukeylambda.pdf(
                            np.linspace(min(data), max(data)), *curve_args
                        ),
                    )
                elif plot:
                    self.plot_percentiles(
                        labels=[output], plot=[output], normalize=False, flip=True
                    )
                    plt.plot(
                        np.linspace(min(data), max(data)),
                        scipy.tukeylambda.cdf(
                            np.linspace(min(data), max(data)), *curve_args
                        ),
                    )
                ...
                if pdf and plot:
                    ...
                    plt.plot(
                        np.linspace(min(data), max(data)),
                        scipy.expon.pdf(np.linspace(min(data), max(data)), *curve_args),
                    )
                elif plot:
                    self.plot_percentiles(
                        labels=[output], plot=[output], normalize=False, flip=True
                    ...
                    plt.plot(
                        np.linspace(min(data), max(data)),
                        scipy.expon.cdf(np.linspace(min(data), max(data)), *curve_args),
                    )
                ...
                if pdf and plot:
                    ...
                    plt.plot(
                        np.linspace(min(data), max(data)),
                        function.pdf(np.linspace(min(data), max(data)), *curve_args),
                    )
                elif plot:
                    self.plot_percentiles(
                        labels=[output], plot=[output], normalize=False, flip=True
                    ...
                    plt.plot(
                        np.linspace(min(data), max(data)),
                        function.cdf(np.linspace(min(data), max(data)), *curve_args),
                    )
    ...
    def plot_percentiles(
        ...
        xlabel = "Percentile" if not xlabel else xlabel
        ylabel = "Normalized Value" if not ylabel else ylabel
        flip = True if pdf else flip
        plotting_dict = {}
        means = {}
        for output in plot:
            means[output] = list(self.statistics["statistics"][output]["value"])[0]
            plotting_dict[output] = {
                "percentiles": self.statistics["confidence_intervals"],
        ...
        for label, data in plotting_dict.items():
            if flip:
                percentiles = (
                    ...
                    + [
                        (data["percentiles"][i] - data["percentiles"][i - 1])
                        / (data["values"][i] - data["values"][i - 1])
                        for i in range(1, len(data["percentiles"]))
                    ]
                )
                ...
                ax.plot(
                    data["percentiles"], data["values"], label=label, marker=markers[i]
                )
            i += 1
        if labels:
            ax.legend(
    ...
    def plot_histogram(
        ...
        for outp in plot:
            histogram_dict[outp] = (
                list(self.matrix["inputs"][outp])
                if plot_inp
                else list(self.matrix["outputs"][outp])
            )
            if normalize:
                if factor == "mean":
                    ...
                    histogram_dict[outp] = [
                        (i / mean - 1) for i in histogram_dict[outp]
                    ]
                elif factor == "min":
                    xlabel = "(X-Min)/Min" if not xlabel else xlabel
                    minima = np.min(histogram_dict[outp])
                    histogram_dict[outp] = [
                        ((i - minima) / minima) for i in histogram_dict[outp]
                    ]
                elif factor == "median":
                    xlabel = "(X-Median)/Median" if not xlabel else xlabel
                    median = np.median(histogram_dict[outp])
                    histogram_dict[outp] = [
                        ((i - median) / median) for i in histogram_dict[outp]
                    ]

        fig, ax = plt.subplots(1, 1, figsize=(6, 4))
        ax.set_title("Histogram Distributions")
        ax.set_xlabel(xlabel)
        ax.set_ylabel(ylabel)
        ax.set_facecolor("aliceblue")
        ax.grid(
    ...
    def plot_violin(
        ...
        viola = ax.violinplot(
            vio_dict.values(), positions, showmeans=True, showextrema=True
        )
    ...
    def plot_sobol(
        ...
        outp="",
        order="FIRST_ORDER",
        inp_labels=["Mobile T Flux", "Cool. T Conc.", "Heat Flux", "Cool. Temp."],
        outp_labels=[
            "H3 Permeation",
            "H3 FLux",
            "Coo. Heat Flux",
        ...
        i = 0
        for inp in self.inputs:
            ax[i].set_ylabel(f"{inp_labels[i]}")
            ...
            y = np.asarray(
                [self.sobol[order][outp]["mean"][inp] for outp in self.sobol["outputs"]]
            )
            # print(y)
            n_err = np.asarray(
                [self.sobol[order][outp][lci][inp] for outp in self.sobol["outputs"]]
            )
            n_err = np.abs(np.subtract(n_err, y))
            p_err = np.asarray(
                [self.sobol[order][outp][uci][inp] for outp in self.sobol["outputs"]]
            )
            p_err = np.abs(np.subtract(p_err, y))

            ax[i].errorbar(
                x, y, yerr=[n_err, p_err], linewidth=0, elinewidth=1, marker="o"
            )
            i += 1
    ...
    def sobol_heat_map(
        self,
        outp="max_temperature_CuCrZr",
        outp_label="Tritium Permeation",
        xlabels=[
            "Tritium Flux",
            "Coolant Tritium \nConcentration",
            "Heat Flux",
        ...
        new_df = {}
        operating_df = self.sobol["SECOND_ORDER"][outp]
        rows = list(operating_df.index)
        for row in rows:
            new_df[row] = {}
            i = 0
            for subrow in rows:
    ...
    def confidence_interval_heatmap(
        self,
        order="FIRST_ORDER",
        outp="max_temperature_CuCrZr",
        outp_label="CCZ Maximum Temperature",
        xlabels=[
            "Tritium Flux",
            "Coolant Tritium \nConcentration",
            "Heat Flux",
        ...
        # Construct new dataframe
        ylabels = [min_cf, "mean", max_cf]
        title = title if title else outp_label
        new_df = {}
        operating_df = self.sobol[order][outp]
        rows = list(operating_df.index)
        for row in rows:
            new_df[row] = {
                min_cf: operating_df[min_cf][row],
                "Mean": operating_df["mean"][row],
                max_cf: operating_df[max_cf][row],
            }
        new_df = pd.DataFrame(new_df)
        if heatmap:
            seaborn.heatmap(
            ...
            styler = new_df.style.format(precision=3)
            # new_df
        return new_df
    ...
    def confidence_interval_table(
        ...
        for inp in self.inputs:
            ...
            for n_inp in range(0, len(self.inputs)):
                ...
                df[inp][
                    n_inp
                ] += f"{np.round(self.sobol["SECOND_ORDER"][outp]["mean"][inp][n_inp], precision)} "
                df[inp][
                    n_inp
                ] += f"({np.round(self.sobol["SECOND_ORDER"][outp][min_cf][inp][n_inp], precision)},"
                df[inp][
                    n_inp
                ] += f"{np.round(self.sobol["SECOND_ORDER"][outp][max_cf][inp][n_inp], precision)})"
        df = pd.DataFrame(df)
    ...
    def plot_inp_out(
        ...
        xlabel=None,
        ylabel=None,
        curve_fit_function=lambda x, a, b: a + b * x,
        guess_args=[0.0, 1.0],
    ):
        xlabel = inp if not xlabel else xlabel
        ylabel = outp if not ylabel else ylabel
        ...
        if curve_fit_function:
            fit_args, pcov = optimize.curve_fit(
                curve_fit_function, x_vector, y_vector, p0=guess_args
            )
...
def Exponential_Kernel(
    vector, *args, pre_exponential_factor=0, frequency=0, power=-1, **kwargs
):
    exponential_vector = pre_exponential_factor * np.exp(
        (frequency * np.power(vector, power))
    )
...
A2.get_sobol_total()
SS.get_sobol_total()
A1.get_statistics(), A2.get_statistics()
SS.get_statistics()
