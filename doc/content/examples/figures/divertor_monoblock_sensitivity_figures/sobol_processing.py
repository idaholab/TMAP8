...
class SOBOL:
    def __init__(
        ...
        inputs=[],
        time_step=0,
        matrix_reporter_name="matrix",
        sobol_reporter_name="sobol",
        statistics_reporter_name="stats",
        results_transfer_name="results",
    ):
        ...
        if inputs and inputs != []:
            ...
            for key in self.reference_dict["time_steps"][self.time_step].keys():
                if f"{matrix_reporter_name}/" in key:
                    inputs.append(f"{key.split('/')[-2]}/{key.split('/')[-1]}")
            self.inputs = inputs

        if inputs == []:
            for key in self.reference_dict["time_steps"][self.time_step][
                matrix_reporter_name
            ].keys():
        ...
        self.sobol = {
            "confidence_intervals": self.reference_dict["reporters"][
                sobol_reporter_name
            ]["confidence_intervals"]["levels"],
            "number_of_samples": self.reference_dict["reporters"][sobol_reporter_name][
                "confidence_intervals"
            ]["replicates"],
            "inputs": inputs,
            "number_of_inputs": self.reference_dict["reporters"][sobol_reporter_name][
                "num_params"
            ],
            "outputs": [
                (i.split(":"))[-2]
                for i in self.reference_dict["reporters"][sobol_reporter_name][
                    "values"
                ].keys()
            ],
            "number_of_outputs": len(
                self.reference_dict["reporters"][sobol_reporter_name]["values"].keys()
            ),
        }
        self.statistics = {
            "confidence_intervals": self.reference_dict["reporters"][
            ...
            "outputs": [],
            "number_of_outputs": len(
                self.reference_dict["reporters"][statistics_reporter_name].keys()
            ),
        }
        stat_outs = []
        ...
        for output in self.sobol["outputs"]:
            ...
            for inp in self.sobol["inputs"]:
                sobol_first[output]["mean"][inp] = self.reference_dict["time_steps"][
                    ...
                    f"{self.results_transfer}_results:{output}:value"
                ][
                    "FIRST_ORDER"
                ][
                    0
    ...
    def get_statistics(self):
        ...
        for output in self.statistics["outputs"]:
            ...
            self.statistics["statistics"][output]["value"][
                "mean"
            ] = self.reference_dict["time_steps"][self.time_step][
                self.statistics_reporter
            ][
                f"{self.results_transfer}_results:{output}:value_MEAN"
            ][
                0
            ]
            self.statistics["statistics"][output]["stddev"][
                ...
                f"{self.results_transfer}_results:{output}:value_STDDEV"
            ][
                0
            ]
            i = 0
    ...
    def get_matrix(self):
        ...
        for inp in inputs:
            try:
                inp_matrix[inp] = self.reference_dict["time_steps"][self.time_step][
                    f"{self.matrix_reporter}/{inp}"
                ]["value"]
            except:
                try:
                    inp_matrix[inp] = self.reference_dict["time_steps"][self.time_step][
                        self.matrix_reporter
                    ][inp]
                except:
        ...
        for outp in outputs:
            outp_matrix[outp] = self.reference_dict["time_steps"][self.time_step][
                self.matrix_reporter
            ][f"{self.matrix_reporter}_results:{outp}:value"]
    ...
    def estimate_state_frequency(
        ...
        path = (
            path
            if path
            else f"frequency_of_{state_limit}_for{output}_by_{"matrix" if use_matrix else function}.png"
        )
        if use_matrix:
            print(
