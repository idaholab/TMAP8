...
This validation case is based on the natural-oxide and thin-oxide experiments reported in [!cite](Kremer2022oxide). The model considered here contains four oxygen-field configurations that use the same tungsten diffusion, trapping, and surface-release formulation with front-surface oxygen inventories corresponding to 1 nm, 5 nm, 10 nm, and 15 nm oxide layers. The experimental study measured deuterium thermal desorption spectra from tungsten samples with a natural oxide layer and with electrochemically grown oxide layers between 5 nm and 100 nm.
...
The reference sample history is taken from the natural-oxide experiment in [!cite](Kremer2022oxide). In the experiment, the tungsten specimen is first prepared with a self-damaged near-surface region, then loaded with deuterium so that the retained inventory is concentrated in the first few micrometers of the sample. The natural-oxide input represents the front surface through a thin 1 nm oxygen concentration field, while the 5 nm, 10 nm, and 15 nm companion inputs use the same deuterium preload and tungsten trapping model but initialize that oxygen field across the corresponding front-oxide thickness instead.
...
| $K_r^{D_2O}$ | D$_2$O surface-release prefactor | 6.0 $\times 10^{-16}$ | m$^4$/at/s | Current phenomenological surface-release parameter |
| $E_r^{D_2O}$ | D$_2$O surface-release activation energy | 0.40 | eV | Current phenomenological surface-release parameter |

## Results

The four oxygen-field inputs use the same scaled six-trap `val-2f` formulation: a 1 nm natural-oxide baseline and companion 5 nm, 10 nm, and 15 nm oxide cases. All solves use the same adimensional mobile transport and `SpeciesTrappingPhysics` syntax in tungsten, while the comparison script converts the results back to physical units. The script overlays every simulated case for which output files are available against the matching experimental `HD + D_2` and `HDO + D_2O` curves, while the inventory and profile figures report representative internal-state information from the simulated cases.
