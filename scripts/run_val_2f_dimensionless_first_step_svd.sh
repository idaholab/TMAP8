...
exec "${BINARY}" \
  ...
  "Executioner/petsc_options=-pc_svd_monitor" \
  Executioner/petsc_options_iname='-pc_type -snes_type' \
  Executioner/petsc_options_value='svd      vinewtonrsls' \
  Outputs/file_base=val-2f-dimensionless_first_step_svd_out
