...
def main() -> None:
    ...
    diffusion_rate_ref = arrhenius(
        diffusion_prefactor_um2_s, diffusion_energy_ev, temperature_ref
    )
    diffusion_time_reference = length_reference_um**2 / diffusion_rate_ref

    release_effective_rates = {
        name: arrhenius(
            detrapping_prefactor_s, detrapping_energies_ev[name], temperature_ref
        )
        for name in detrapping_energies_ev
    }
    release_time_scales = {
        name: 1.0 / rate for name, rate in release_effective_rates.items()
    }
    ...
    dimensionless_trapping_rates = {
        name: trapping_prefactor_s * time_reference for name in trapping_energies_ev
    }
