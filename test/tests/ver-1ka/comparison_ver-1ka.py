...
ax.plot(
    TMAP8_time / 3600, analytical_pressure, label=r"Analytical", c="k", linestyle="--"
)
...
ax.text(
    0.05,
    0.95,
    "$P_{T_2}$ RMSPE = %.2f " % err_percent_pressure + "%",
    fontweight="bold",
)

# Format the y-axis to use scientific notation
plt.gca().yaxis.set_major_formatter(
    plt.FuncFormatter(lambda val, pos: "{:.1e}".format(val))
)
