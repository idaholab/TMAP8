...
pressure_T2_high = '${units ${fparse 1e-10 * pressure_atm} Pa}' # default standalone upstream T2 pressure
...
# chemical_reaction — optimized BCY parameters (n20_faster, 72 steps), identical to OPTIMIZED_PARAMS
# in tritium_transport_PCC/scripts/compare_original_optimized_results_flux.py (the published
# validation set used for the "after optimization" flux-vs-voltage figure).
...
diffusivity_OT_prefactor = '${units ${fparse 2.03 * 10 ^ diffusivity_OT_prefactor_exponent / sqrt(3)} m^2/s -> nm^2/s}'
...
# In the DIR coupling the parent (dir_pcc_fuel_cycle.i) sets A_membrane per instance via cli_args:
# VP (vacuum-pump) membrane = 6e-2 m^2, FCU (fuel-clean-up) membrane = 3e-2 m^2. The value below is
# only the standalone fallback used when this sub-app is run on its own.
A_membrane = '${units 3e-2 m^2}'      # membrane active area [m^2] (parent overrides per DIR stream)
...
[Outputs]
  exodus = false
  console = false
  # CSV suppressed by the parent (Outputs/csv/enable=false via cli_args) since two DIR membrane
  # instances share this file; the DIR fluxes are recorded in the parent fuel-cycle output instead.
  [csv]
    type = CSV
  []
[]
