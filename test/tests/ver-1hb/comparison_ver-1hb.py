...
P_1T, P_2T, P_1D, P_2D = get_analytical_solution(
    tmap8_sol_time)
# ===============================================================================
...
alpha = 0.6
ax.plot(tmap8_sol_time, tmap8_sol_P1_T,
        label=r"$P_1T$ TMAP8", c='tab:pink', alpha=alpha)
ax.plot(tmap8_sol_time, tmap8_sol_P2_T,
        label=r"$P_2T$ TMAP8", c='tab:blue', alpha=alpha)
ax.plot(tmap8_sol_time, P_1T,
        label=r"$P_1T$ Analytical", c='m', linestyle='--')
ax.plot(tmap8_sol_time, P_2T,
        label=r"$P_2T$ Analytical", c='b', linestyle='--')
...
ax.text(10, 0.625, '($P_1T$) RMSPE = %.2f ' %
        err_percent_P_1T+'%', fontweight='bold', color='tab:pink')
RMSE_P_2T = np.linalg.norm(tmap8_sol_P2_T-P_2T)
err_percent_P_2T = RMSE_P_2T*100/np.mean(P_2T)
ax.text(10, 0.375, '($P_2T$) RMSPE = %.2f ' %
        err_percent_P_2T+'%', fontweight='bold', color='tab:blue')

plt.savefig('ver-1hb_comparison_pressure_tritium.png',
...
ax = fig.add_subplot(gs[0])
alpha = 0.6
ax.plot(tmap8_sol_time, tmap8_sol_P1_D,
        label=r"$P_1D$ TMAP8", c='tab:pink', alpha=alpha)
ax.plot(tmap8_sol_time, tmap8_sol_P2_D,
        label=r"$P_2D$ TMAP8", c='tab:blue', alpha=alpha)
ax.plot(tmap8_sol_time, P_1D,
        label=r"$P_1D$ Analytical", c='m', linestyle='--')
ax.plot(tmap8_sol_time, P_2D,
        label=r"$P_2D$ Analytical", c='b', linestyle='--')
...
ax.text(10, 0.375, '($P_1D$) RMSPE = %.2f ' %
        err_percent_P_1D+'%', fontweight='bold', color='tab:pink')
RMSE_P_2D = np.linalg.norm(tmap8_sol_P2_D-P_2D)
err_percent_P_2D = RMSE_P_2D*100/np.mean(P_2D)
ax.text(10, 0.625, '($P_2D$) RMSPE = %.2f ' %
        err_percent_P_2D+'%', fontweight='bold', color='tab:blue')

plt.savefig('ver-1hb_comparison_pressure_deuterium.png',
            bbox_inches='tight', dpi=300)
plt.close(fig)
