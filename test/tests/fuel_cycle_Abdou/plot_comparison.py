...
bz_pts = read_csv_from_TMAP8(
    "abdou2021bz.csv", ["time_[s]", "breeding_zone_[kg]"], " "
)  # read csv file
tes_pts = read_csv_from_TMAP8(
    "abdou2021tes.csv", ["time_[s]", "tritium_extraction_system_[kg]"], " "
)  # read csv file
sto_pts = read_csv_from_TMAP8(
    "abdou2021sto.csv", ["time_[s]", "storage_[kg]"], " "
)  # read csv file
iss_pts = read_csv_from_TMAP8(
    "abdou2021iss.csv", ["time_[s]", "isotope_separation_system_[kg]"], " "
)  # read csv file
...
linestyles = {"T_02_TES": "--", "T_01_BZ": ":", "T_11_storage": "-", "T_09_ISS": "-."}
for i, x_key in enumerate(x_keys[1:]):
    ax.plot(
        df[x_keys.index("time")],
