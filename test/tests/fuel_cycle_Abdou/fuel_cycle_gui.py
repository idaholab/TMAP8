...
class fuel_cycle_form(tk.Tk):
    def __init__(self, interval=1 / 120, tmap8_path=None, headless=False):
        ...
        if tmap8_path is None:
            check_paths = [
                os.path.join(dir_path, "..", "..", "..", "tmap8-opt"),
        ...
        filtstring = [
            x[: x.find("#")].strip() if "#" in x else x
            for x in instring[start:end].split("\n")
        ]
        self.filtstring = "\n".join(
            [x.strip() for x in filtstring if len(x.strip()) > 1]
        )
        self.matches = [
            (m.span(), m.groups()) for m in pattern.finditer(self.filtstring)
        ]
        labels = []
        self.entries = []
        self.plot_vars = []
        ...
        scrollbar = tk.Scrollbar(
            self.param_frame, orient="vertical", command=self.scroll_can.yview
        )
        ...
        self.scroll_can.create_window(
            (8, 4), window=self.scrollable, tags="self.frame", anchor="nw"
        )
        ...
        self.float_validator = (
            self.register(self.float_validation),
            "%d",
            "%i",
            "%P",
            "%s",
        ...
        self.time_combobox = ttk.Combobox(
            self.scrollable, width=20, textvariable=self.time_unit
        )
        ...
        for label, x in zip(self.matches, self.input_entries):
            try:
                ...
                x_str = x.get()
                try:
                    self.labeldict[label[1][0]] = float(
                        x.get().replace("e", "").replace("-", "")
                    )
                except ValueError:
                    raise ValueError(
                        "Non-numeric value in parameter definition during creation"
                    )
                    self.destroy()
    ...
    def test_compare(self):
        check_one = np.genfromtxt(
            self.tmpfile[:-2] + "_out.csv", skip_header=1, delimiter=","
        )
        ...
        for match in self.matches:
            ...
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        "Non-numeric value in parameter definition {:s}".format(
                            match[1][0]
                        )
                    )
                    self.destroy()
    ...
    def buttonClick(self):
        ...
        if self.first_run:
            ...
            scrollbar = tk.Scrollbar(
                self.cbframe, orient="vertical", command=self.cbcanvas.yview
            )
            ...
            self.cbcanvas.create_window(
                (8, 4), window=self.cblable, tags="self.frame", anchor="nw"
            )
        ...
        for pli in self.plot_ints:
            if pli.get() > 0:
                self.lines[i].set_xdata(
                    self.iz_data[:, 0] / self.time_divisors[self.time_unit.get()]
                )
    ...
    def change_scale(self, *arg):
        ...
        xlims[0] = (
            xlims[0] * self.old_timescale / self.time_divisors[self.time_unit.get()]
        )
        xlims[1] = (
            xlims[1] * self.old_timescale / self.time_divisors[self.time_unit.get()]
        )
        for line in self.lines:
            data = (
                line.get_xdata()
                * self.old_timescale
                / self.time_divisors[self.time_unit.get()]
            )
            line.set_xdata(data)
        self.old_timescale = self.time_divisors[self.time_unit.get()]
...
parser.add_argument(
    "--test",
    help="for use in TMAP8 testing to verify successful script execution\
        No GUI will be shown when using this flag",
    action="store_true",
)
