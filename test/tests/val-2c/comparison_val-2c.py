...
def common_times_generator(time_exp, value_exp, time_solution, value_solution):
    ...
    for i in range(len(time_solution)):
        ...
        if (
            time_solution[i] < time_exp[len(time_exp) - 1]
        ):  # there is no experimental data beyond this time value
            index_exp = next(
                k for k, value in enumerate(time_exp) if value > time_solution[i]
            )
            if (index_exp >= 1) and (
                index_exp % 2
            ):  # the current time falls within a range where there is experimental data (index_exp is odd), and it is sampled
    ...
    return (
        np.array(time_sample),
        np.array(value_exp_sampled),
        np.array(value_solution_sampled),
    )
...
def sample_solution(time_stamps, time_solution, value_solution):
    ...
    for i in range(len(time_stamps)):
        ...
        if (
            time_stamps[i + 1] > time_solution[len(time_solution) - 1]
        ):  ##### -1 does not work for this
            index_1 = len(time_solution) - 2
            index_2 = len(time_solution) - 1
        else:
            index = next(
                k
                for k, value in enumerate(time_solution)
        ...
        value_solution_samples[i] = (
            value_solution[index_2] - value_solution[index_1]
        ) / (time_solution[index_2] - time_solution[index_1]) * (
            time_stamps[i + 1] - time_solution[index_1]
        ) + value_solution[
            index_1
        ]
...
tmap8_t2_enclosure_concentration_delay = tmap8_prediction_delay[
    "t2_enclosure_edge_concentration"
]
tmap8_hto_enclosure_concentration_delay = tmap8_prediction_delay[
    "hto_enclosure_edge_concentration"
]
...
tmap8_t2_enclosure_concentration_delay_calibrated = tmap8_prediction_delay_calibrated[
    "t2_enclosure_edge_concentration"
]
tmap8_hto_enclosure_concentration_delay_calibrated = tmap8_prediction_delay_calibrated[
    "hto_enclosure_edge_concentration"
]
...
parameters = [
    "reaction_rate",
    "diffusivity_elemental_tritium",
    "diffusivity_tritiated_water",
    "log_solubility_elemental_tritium",
    "log_solubility_tritiated_water",
...
for i in np.arange(n_steps):
    ...
    outputs[ind1] = np.array(
        data["time_steps"][i.astype(int)]["adaptive_MC"]["output_required"]
    )
...
lines = [
    parameters[i] + " = " + str(calibrated_inputs[i]) + "\n" for i in range(n_variables)
]
...
indices_parameters_normal = [
    0,
    1,
    2,
    5,
]  # indices of the parameters vector corresponding to parameters following a normal distribution
...
parameters_normal_average = [
    2.8 * 2.0e-10 * conversion_Ci_atom * m_to_mum**3,
    4.0e-12 * m_to_mum**2 * (1.0 - 0.1),
    1.0e-14 * m_to_mum**2 * (1.0 + 0.4),
    3 * 60 * 60,
]
parameters_normal_std = [
    5 / 100 * parameters_normal_average[0],
...
for i in range(len(parameters_normal)):
    norm_optimized_input = (
        calibrated_inputs_normal[i] - parameters_normal_average[i]
    ) / parameters_normal_std[i]
    ax1.plot(
        [norm_optimized_input, norm_optimized_input],
        y_limit,
        "--",
        label=parameters_legend_normal[i],
    )
...
indices_parameters_uniform = [
    3,
    4,
]  # indices of the parameters vector corresponding to parameters following a uniform distribution
...
parameters_uniform_min = [
    np.log(1e-4 * solubility_elemental_tritium),
    np.log(1e-4 * solubility_tritiated_water),
]
parameters_uniform_max = [
    np.log(1e-3 * solubility_elemental_tritium),
...
for i in range(len(parameters_uniform)):
    ...
    uniform_distribution_y = [
        parameters_uniform_min[i] <= x <= parameters_uniform_max[i]
        for x in uniform_distribution_x
    ]
    ax1.plot(uniform_distribution_x, uniform_distribution_y, "-", color="k")
    ax1.plot(
        [calibrated_inputs_uniform[i], calibrated_inputs_uniform[i]],
        y_limit,
        "--",
        label=parameters_legend_uniform[i],
    )
...
if "/tmap8/doc/" in script_folder.lower():  # if in documentation folder
    csv_folder = (
        "../../../../test/tests/val-2c/gold/Experimental_data_HTO_concentration.csv"
    )
else:  # if in test folder
...
experimental_hto_enclosure_concentration = experimental_data_hto[
    "Concentration (Ci/m3)"
]

if "/tmap8/doc/" in script_folder.lower():  # if in documentation folder
    csv_folder = (
        "../../../../test/tests/val-2c/gold/Experimental_data_T2_concentration.csv"
    )
else:  # if in test folder
...
tmap8_t2_enclosure_concentration_Ci = (
    tmap8_t2_enclosure_concentration * 2 * conversion_Ci_atom * length_scale**3
)  # molecules/microns^3 -> Ci/m^3 - the factor 2 is because there are 2 T atoms per T2 molecule
tmap8_hto_enclosure_concentration_Ci = (
    tmap8_hto_enclosure_concentration * conversion_Ci_atom * length_scale**3
)  # molecules/microns^3 -> Ci/m^3

tmap8_time_delay = tmap8_time_delay / time_scale  # s -> h
tmap8_t2_enclosure_concentration_Ci_delay = (
    tmap8_t2_enclosure_concentration_delay * 2 * conversion_Ci_atom * length_scale**3
)  # molecules/microns^3 -> Ci/m^3 - the factor 2 is because there are 2 T atoms per T2 molecule
tmap8_hto_enclosure_concentration_Ci_delay = (
    tmap8_hto_enclosure_concentration_delay * conversion_Ci_atom * length_scale**3
)  # molecules/microns^3 -> Ci/m^3

tmap8_time_delay_calibrated = tmap8_time_delay_calibrated / time_scale  # s -> h
tmap8_t2_enclosure_concentration_Ci_delay_calibrated = (
    tmap8_t2_enclosure_concentration_delay_calibrated
    * 2
    * conversion_Ci_atom
    * length_scale**3
)  # molecules/microns^3 -> Ci/m^3 - the factor 2 is because there are 2 T atoms per T2 molecule
tmap8_hto_enclosure_concentration_Ci_delay_calibrated = (
    tmap8_hto_enclosure_concentration_delay_calibrated
    * conversion_Ci_atom
    * length_scale**3
)  # molecules/microns^3 -> Ci/m^3
...
ax.plot(
    tmap8_time,
    tmap8_hto_enclosure_concentration_Ci,
    label=r"TMAP8 HTO - immediate injection",
    c="tab:gray",
    linestyle="--",
)
ax.plot(
    tmap8_time_delay,
    tmap8_hto_enclosure_concentration_Ci_delay,
    label=r"TMAP8 HTO - delayed injection",
    c="tab:gray",
    linestyle=":",
)
ax.plot(
    ...
    tmap8_hto_enclosure_concentration_Ci_delay_calibrated,
    label=r"TMAP8 HTO - delayed injection - calibrated",
    c="tab:red",
    linestyle=":",
)
for i in range(int(len(experimental_time_hto) / 2)):
    if i == 0:
        ax.plot(
            [experimental_time_hto[2 * i], experimental_time_hto[2 * i + 1]],
            [
                experimental_hto_enclosure_concentration[2 * i],
                experimental_hto_enclosure_concentration[2 * i + 1],
            ],
            label=r"Experimental data HTO",
            c="k",
            linestyle="-",
        )
    else:
...
(
    ...
    tmap8_hto_enclosure_concentration_Ci_sampled,
) = common_times_generator(
    experimental_time_hto,
    experimental_hto_enclosure_concentration,
    tmap8_time,
    tmap8_hto_enclosure_concentration_Ci,
)
RMSE = np.sqrt(
...
(
    time_sample_delay,
    experimental_hto_enclosure_concentration_delay_sampled,
    tmap8_hto_enclosure_concentration_Ci_delay_sampled,
) = common_times_generator(
    experimental_time_hto,
    experimental_hto_enclosure_concentration,
    tmap8_time_delay,
...
(
    time_sample_delay_calibrated,
    experimental_hto_enclosure_concentration_delay_calibrated_sampled,
    tmap8_hto_enclosure_concentration_Ci_delay_calibrated_sampled,
) = common_times_generator(
    experimental_time_hto,
    experimental_hto_enclosure_concentration,
    tmap8_time_delay_calibrated,
...
RMSPE = (
    RMSE
    * 100
    / np.mean(experimental_hto_enclosure_concentration_delay_calibrated_sampled)
)
ax.text(
    11,
    4e-6,
    "(delay) RMSPE (calibrated) = %.2f " % RMSPE + "%",
    fontweight="bold",
    c="tab:red",
)
...
ax.plot(
    tmap8_time,
    tmap8_t2_enclosure_concentration_Ci,
    label=r"TMAP8 T$_2$ - immediate injection",
    c="tab:gray",
    linestyle="--",
)
ax.plot(
    tmap8_time_delay,
    tmap8_t2_enclosure_concentration_Ci_delay,
    label=r"TMAP8 T$_2$ - delayed injection",
    c="tab:gray",
    linestyle=":",
)
ax.plot(
    tmap8_time_delay_calibrated,
    tmap8_t2_enclosure_concentration_Ci_delay_calibrated,
    label=r"TMAP8 T$_2$ - delayed injection - calibrated",
    c="tab:red",
    linestyle=":",
)
ax.scatter(
    experimental_time_t2[0],
    experimental_t2_enclosure_concentration[0],
    label=r"Experimental data T$_2$ (amount injected)",
    c="k",
    alpha=0.6,
)
ax.scatter(
    experimental_time_t2[1:],
    experimental_t2_enclosure_concentration[1:],
    label=r"Experimental data T$_2$",
    c="k",
)
ax.set_xlabel("Time (hr)")
...
# calculate RMSPE
tmap8_t2_enclosure_concentration_Ci_sampled = sample_solution(
    experimental_time_t2[1:], tmap8_time, tmap8_t2_enclosure_concentration_Ci
)
RMSE = np.sqrt(
    np.mean(
        (
            tmap8_t2_enclosure_concentration_Ci_sampled
            - experimental_t2_enclosure_concentration[1:]
        )
        ** 2
    )
...
tmap8_t2_enclosure_concentration_Ci_delay_sampled = sample_solution(
    experimental_time_t2[1:],
    tmap8_time_delay,
    tmap8_t2_enclosure_concentration_Ci_delay,
)
RMSE = np.sqrt(
    np.mean(
        (
            tmap8_t2_enclosure_concentration_Ci_delay_sampled
            - experimental_t2_enclosure_concentration[1:]
        )
        ** 2
    )
...
tmap8_t2_enclosure_concentration_Ci_delay_calibrated_sampled = sample_solution(
    experimental_time_t2[1:],
    tmap8_time_delay_calibrated,
    tmap8_t2_enclosure_concentration_Ci_delay_calibrated,
)
RMSE = np.sqrt(
    np.mean(
        (
            tmap8_t2_enclosure_concentration_Ci_delay_calibrated_sampled
            - experimental_t2_enclosure_concentration[1:]
        )
        ** 2
    )
