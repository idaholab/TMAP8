...
def analytical_solution_concentration(
    time,
    location,
    ...
    # first term
    first_term = 0.5 * special.erfc(
        (temperature_gradient * diffusivity * soret_coefficient * time + location)
        / two_sqrt_Dt
    )
    # second term
    second_term = (
    ...
    # final result
    concentration_result = (first_term + second_term) * (
        concentration_left - concentration_initial
    ) + concentration_initial
...
if "/tmap8/doc/" in script_folder.lower():  # if in documentation folder
    csv_folder = (
        "../../../../test/tests/ver-1l/gold/ver-1l_vector_postproc_line_0163.csv"
    )
else:  # if in test folder
...
ax.plot(
    tmap8_at_10m_time, concentration_at_10m, label=r"Analytical", c="k", linestyle="--"
)
...
ax.minorticks_on()
plt.savefig(
    "ver-1l_comparison_analytical_concentration_location.png",
    bbox_inches="tight",
    dpi=300,
)
plt.close(fig)
...
ax = fig.add_subplot(gs[0])
ax.plot(
    tmap8_at_100s_location, tmap8_at_100s_concentration, label=r"TMAP8", c="tab:gray"
)
ax.plot(
    analytical_location[:node_number],
    concentration_at_100s,
    label=r"Analytical",
    c="k",
    linestyle="--",
)
...
ax.minorticks_on()
plt.savefig(
    "ver-1l_comparison_analytical_concentration_time.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
