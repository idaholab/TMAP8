...
# Steel-only simulation data for comparison
steel_only_total_mass_steel = data_steel_only['3d_mass_in_domain']
t_steel = data_steel_only['time']
# SRNL data for validation
SRNL_time = SRNL_data['Dose (MGy)']*365.25/65.21904 # Assuming 124 Gy/min dosage in Cobalt 60 irraditator
SRNL_total_mass_gas = 2 * SRNL_data['Cum. H2 yield (μmol)'] # atomic H
SRNL_partial_pressure = 1e3* SRNL_data["Gas pressure (kPa)"]*SRNL_data["H2 gas fraction (%)"]/100 # Pa

# Map simulation results to experimental data for validation
...
mapped_pressure = numerical_solution_on_experiment_input(SRNL_time, t, H_partial_pressure_interface)
RMSE = np.sqrt(np.mean((mapped_pressure - SRNL_partial_pressure)**2))
RMSPE = RMSE*100/np.mean(SRNL_partial_pressure)
plt.text(t.iloc[-1]/2,H_partial_pressure_interface.iloc[-1]/4, 'RMSPE = %.2f '%RMSPE+'%',fontweight='bold')
plt.ylabel(r'H_2 Partial Pressure (Pa)')
plt.xlabel('Time (Days)')
plt.title(f'Interface Partial Pressure: SRNL Data vs. Simulation')
plt.xlim(0)
plt.legend()
plt.ylim(0)
plt.grid(True)
...
if len(t_steel) < len(t): # Error handling if different number of timesteps are taken between two simulations
    total_mass_steel_RMSPE = numerical_solution_on_experiment_input(t_steel,t,total_mass_steel)
    ...
    steel_only_total_mass_steel_RMSPE = numerical_solution_on_experiment_input(t,t_steel,steel_only_total_mass_steel)
...
plt.text(t.iloc[-1]/2,total_mass_steel.iloc[-1]/4, 'RMSPE = %.2f '%RMSPE+'%',fontweight='bold')
...
simulation_mapped_total_mass_gas = numerical_solution_on_experiment_input(SRNL_time, t, total_mass_gas) # Map simulation results to experimental data
RMSE = np.sqrt(np.mean((simulation_mapped_total_mass_gas - SRNL_total_mass_gas)**2))
RMSPE = RMSE*100/np.mean(SRNL_total_mass_gas)
plt.text(t.iloc[-1]/2,total_mass_gas.iloc[-1]/4, 'RMSPE = %.2f '%RMSPE+'%',fontweight='bold')
plt.legend()
...
fig = plt.figure(figsize=(10, 6))
plt.plot(t, time_integrated_flux + total_generation, label = 'Accumulated Boundary Flux + Source')
...
plt.text(t.iloc[-1]/2,total_mass.iloc[-1]/4, 'RMSPE = %.2f '%RMSPE+'%',fontweight='bold')
plt.xlabel('Time (Days)')
