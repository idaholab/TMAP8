...
def get_analytical_solution(numerical_steps):
    ...
    p_AB_analytical_special = (
        2
        * p0_A2
        * p0_B2
        * (1 - np.exp(-2 * S * K_d * kb * T * numerical_steps / V))
        / (p0_A2 + p0_B2)
    )
...
plt.savefig("ver-1ie_comparison_pressure.png", bbox_inches="tight", dpi=300)
plt.close(fig)
