...
def interpolation_on_expected_input(date_x, data_y, expected_input):
    ...
    return (expected_input - date_x[left_limit]) / (
        date_x[right_limit] - date_x[left_limit]
...
reserve_inventory = (
    tritium_burn_rate / TBE * q * t_res * AF
)  # necessary inventory reserved for off time
...
simulation_results[parameter_names.index("time")] = (
    simulation_results[parameter_names.index("time")] / time_unit
)  # update time unit

# inflection point
inflection_y = np.min(simulation_results[parameter_names.index("T_10_storage")])
inflection_x = simulation_results[parameter_names.index("time")][
    np.argmin(simulation_results[parameter_names.index("T_10_storage")])
]
print(
    f"TMAP8_base: Inflection time = {round(inflection_x,5)} days, and inventory = {round(inflection_y,5)} kg"
)

# ======================== Benchmark data extraction ======================== #
file_name = "inventory_paper_20days.csv"
parameter_names_benchmark = [
    "time [s]",
...
benchmark_results = read_csv_from_TMAP8(
    file_name, parameter_names_benchmark
)  # read csv file
benchmark_results[parameter_names_benchmark.index("time [s]")] = (
    benchmark_results[parameter_names_benchmark.index("time [s]")] / time_unit
)  # update time unit

# inflection point
inflection_y = np.min(
    benchmark_results[parameter_names_benchmark.index("storage inventory [kg]")]
)
inflection_x = benchmark_results[parameter_names_benchmark.index("time [s]")][
    np.argmin(
        benchmark_results[parameter_names_benchmark.index("storage inventory [kg]")]
    )
]
print(
    f"benchmark: Inflection time = {round(inflection_x,5)} days, and inventory = {round(inflection_y,5)} kg"
)
