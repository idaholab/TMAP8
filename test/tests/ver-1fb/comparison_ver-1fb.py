...
def get_analytical_solution(x, time_i):
    ...
    temperature = To + (T1 - To) * (
        1
        - (x / L)
        - (2 / L) * summation_terms(num_summation_terms, x, time_i, alpha, L)
    )
...
ax.plot(
    tmap_x, analytical_temp[0], label=r"Analytical 0.1 seconds", c="k", linestyle="--"
)
ax.plot(
    tmap_x, analytical_temp[1], label=r"Analytical 0.5 seconds", c="r", linestyle="--"
)
ax.plot(
    tmap_x, analytical_temp[2], label=r"Analytical 1.0 seconds", c="b", linestyle="--"
)
ax.plot(
    tmap_x, analytical_temp[3], label=r"Analytical 5.0 seconds", c="c", linestyle="--"
)

ax.set_xlabel("Distance along slab (m)")
