...
# Level 3: Real (H2+H2O) BCY voltage simulation with Joule heating (standard RMSPE)
# Based on jouleheating_Voltage_BCY_real_LEE2005.i
# Uses standard RMSPE: sqrt(mean(((sim-ref)/ref)^2)) with per-point normalization
...
# chemical_reaction - optimized parameters used for val-2l Joule-heating validation
...
[Postprocessors]
  ...
  [relative_error_sq]
    type = ParsedPostprocessor
    pp_names = 'recombination_flux_OT_dry_right'
    expression = '((abs(recombination_flux_OT_dry_right * 1e18) - ${target_flux}) / ${target_flux})^2'
    execute_on = 'INITIAL TIMESTEP_END'
  []
