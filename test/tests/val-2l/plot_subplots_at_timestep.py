...
def subplot_bar_profile(timestep, data_dir="csv_data"):
    ...
    gas_path = os.path.join(
        data_dir, f"verification_RZ_solution_profile_gas_{timestep:04d}.csv"
    )
    steel_path = os.path.join(
        data_dir, f"verification_RZ_solution_profile_steel_{timestep:04d}.csv"
    )
    ...
    total_df = pd.read_csv(
        os.path.join(data_dir, "verification_RZ.csv"), skipinitialspace=True
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Plot gas and steel concentration profiles for a given timestep."
    )
    parser.add_argument(
        "timestep", type=int, help="Timestep number (e.g., 1 for _0001.csv)"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
