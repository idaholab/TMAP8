...
tritium_trapped_inventory_initial = (
    2.8438315780556e25 * slab_height * slab_width
)  # atoms -- integral of the normal distribution over the slab length * area
tritium_mobile_inventory_initial = (
    tritium_mobile_concentration_initial * slab_length * slab_height * slab_width
)  # atoms
...
analytical_tritium = (
    tritium_mobile_inventory_initial + tritium_trapped_inventory_initial
) * np.exp(-decay_rate_constant * tmap8_time)
analytical_helium = (
    tritium_mobile_inventory_initial + tritium_trapped_inventory_initial
) * (1.0 - np.exp(-decay_rate_constant * tmap8_time))
...
ax.plot(
    tmap8_time_years,
    tmap8_tritium_trapped + tmap8_tritium_mobile,
    label=r"$I_{tot}$ - TMAP8",
    c="k",
    alpha=0.5,
)
ax.plot(
    tmap8_time_years,
    analytical_tritium,
    label=r"$I_{tot}$ - Analytical",
    c="k",
    linestyle="--",
)
ax.plot(
    tmap8_time_years,
    tmap8_tritium_trapped,
    label=r"$I_T$ - TMAP8",
    c="tab:blue",
    alpha=0.8,
    ls=":",
...
ax.plot(
    tmap8_time_years, tmap8_helium, label=r"$I_{He}$ - TMAP8", c="tab:red", alpha=0.5
)
ax.plot(
    tmap8_time_years,
    analytical_helium,
    label=r"$I_{He}$ - Analytical",
    c="r",
    linestyle="--",
)
# Root Mean Square Percentage Error calculations
RMSE_Ctot = np.linalg.norm(
    tmap8_tritium_trapped + tmap8_tritium_mobile - analytical_tritium
)
RMSPE_Ctot = RMSE_Ctot * 100 / np.mean(analytical_tritium)
ax.text(
    33,
    0.6e25,
    "$I_{tot}$ RMSPE = %.2f " % RMSPE_Ctot + "%",
    fontweight="bold",
    color="k",
)
...
ax.minorticks_on()
plt.savefig(
    "ver-1jb_comparison_analytical_time_evolution.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
...
if "/tmap8/doc/" in script_folder.lower():  # if in documentation folder
    csv_folder_profile = (
        "../../../../test/tests/ver-1jb/gold/ver-1jb_profile_out_line_0048.csv"
    )
else:  # if in test folder
...
gs = gridspec.GridSpec(1, 1)
ax = fig.add_subplot(gs[0])
tmap8_tritium_trapped_init = (
    trapping_sites_fraction_occupied_initial
    * density_material
    * trapping_sites_atomic_fraction_max
    * np.exp(
        -1
        / 2
        * ((tmap8_position - normal_center_position) / normal_standard_deviation) ** 2
    )
)
ax.plot(
    tmap8_position,
...
ax.plot(
    tmap8_position,
    tmap8_helium,
    label=r"$C_{He}(t=45 \mathrm{years})$",
    c="tab:red",
    alpha=1,
)
...
ax.minorticks_on()
plt.savefig(
    "ver-1jb_equivalent_concentrations_comparison_analytical_time_evolution.png",
    bbox_inches="tight",
    dpi=300,
)
plt.close(fig)
...
if "/tmap8/doc/" in script_folder.lower():  # if in documentation folder
    ...
    csv_folder_profile = (
        "./gold/ver-1jb_equivalent_concentrations_profile_out_line_0048.csv"
    )
tmap8_solution = pd.read_csv(csv_folder_profile)
tmap8_position = tmap8_solution["x"]
tmap8_tritium_mobile = tmap8_solution["tritium_mobile_concentration"]
...
ax.minorticks_on()
plt.savefig(
    "ver-1jb_equivalent_concentrations_profile.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
