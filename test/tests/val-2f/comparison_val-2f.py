...
def extract_tmap_data_desorption(df):
    ...
    if (
        ...
        and "scaled_flux_surface_right_sieverts" in df.columns
    ):
        flux_left = abs(df["scaled_flux_surface_left_sieverts"])
        flux_right = abs(df["scaled_flux_surface_right_sieverts"])
    elif (
        "scaled_flux_surface_left" in df.columns
        and "scaled_flux_surface_right" in df.columns
    ):
...
ax.set(
    xlabel="x (m)",
    ylabel=r"Deuterium source (at/m$^3$/s)",
    xlim=(0, None),
    ylim=(0, None),
)
...
for key, fname in [
    ("default", "val-2f_comparison"),
    ("inf", "val-2f_comparison_inf_recombination"),
]:
    _, temp, _, _, flux_total, *_ = extract_tmap_data_desorption(tmap_data[key])
    interp_flux = numerical_solution_on_experiment_input(
        experiment_temperature, temp, flux_total
    )
    ...
    ax.scatter(
        experiment_temperature, experiment_flux, label="Experiment", color="black"
    )
...
for key, fname in [
    ...
    for i in range(1, len(flux_total)):
        escaping[i] = (
            escaping[i - 1] + 0.5 * (flux_total[i] + flux_total[i - 1]) * dt[i - 1]
        )
    ...
    fig.legend(
        handles=patches, loc="center right", bbox_to_anchor=(0.85, 0.5), fontsize=8
    )
    save_plot(fig, f"{fname}.png")
