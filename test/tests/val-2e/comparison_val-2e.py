...
time_history = np.array(
    [150, 250, 350, 450, 550, 650, 750, 850, 950, 1050, 1150, 1250, 1350, 1900]
)  # s
...
for i in range(len(simulation_file_names)):
    ...
    simulation_results = read_csv_from_TMAP8(
        file_name, parameter_names
    )  # read csv file
    simulation_results[parameter_names.index("flux_surface_right_D")] = (
        -simulation_results[parameter_names.index("flux_surface_right_D")] * 1e12 / Na
    )  # D atoms/mum^2/s -> D2 mol/m^2/s
...
for index in range(len(simulation_results_list)):
    ...
    for i in range(len(simulation_results_list[index][parameter_names.index("time")])):
        if len(time_history_copy) == 0:
            break
        if (
            time_history_copy[0]
            - simulation_results_list[index][parameter_names.index("time")][i]
        ) < dt_maximum:
            tmp_pressure_up_array.append(
                simulation_results_list[index][
                    parameter_names.index("pressure_upstream_D2")
                ][i]
            )
            tmp_pressure_down_array.append(
                simulation_results_list[index][
                    parameter_names.index("pressure_downstream_D2")
                ][i]
            )
            tmp_flux_array.append(
                simulation_results_list[index][
                    parameter_names.index("flux_surface_right_D")
                ][i]
            )
            tmp_time_array.append(
                simulation_results_list[index][parameter_names.index("time")]
            )

            tmp_flux_calculated = (
                (tmp_pressure_down_array[-1] - pressure_enclosure1)
                * Q
                / R
                / T[index]
                / Area
...
experiment_file_names = [
    "experiment_thick_825K.csv",
    "experiment_thin_825K.csv",
    "experiment_thin_865K.csv",
]
...
for i in range(len(experiment_file_names)):
    ...
    experiment_results = read_csv_from_TMAP8(
...
pressure_history_plot = np.array(
    [
        1.20e-4,
        1.20e-4,
        2.41e-4,
...
time_history_plot_mixture = np.array(
    [0, 150, 150, 250, 250, 350, 350, 450, 450, 550, 550, 650, 650, 750, 750, 1900]
)  # s
pressure_history_plot_mixture_D2 = np.array(
    [
        1.8421 - 4,
        1.8421 - 4,
        1e-3,
...
for index in [2, 1, 0]:
    ax.plot(
        ...
        label=f"{label_name[index]} in {label_name_postfix[index]} (TMAP8)",
        c=f"C{index}",
    )
for index in [2, 1, 0]:
    ax.plot(
        experiment_results_list[index][
            experiment_parameter_names.index("Pressure [Pa]")
        ],
        experiment_results_list[index][
            experiment_parameter_names.index("Flux [mol/m^2/s]")
        ],
        signal_list[index],
        label=f"{label_name[index]} (experiment)",
        c=f"C{index}",
    )
...
for index in range(len(simulation_results_list)):
    experiment_input = experiment_results_list[index][
        experiment_parameter_names.index("Pressure [Pa]")
    ]
    experiment_output = experiment_results_list[index][
        experiment_parameter_names.index("Flux [mol/m^2/s]")
    ]
    tmap_flux_for_rmspe = numerical_solution_on_experiment_input(
        experiment_input, pressure_up_array[index], flux_calculated_array[index]
    )
    RMSPE = (
        np.sqrt(
            np.mean(
                ((tmap_flux_for_rmspe - experiment_output) / experiment_output) ** 2
            )
        )
        * 100
    )
...
pressure_history_D = np.array(
    [1.8421e-4, 1e-3, 3e-3, 0.009, 0.027, 0.081, 0.243, 0.729]
)
...
for i in range(len(simulation_file_names)):
    ...
    simulation_results[parameter_names.index("flux_surface_right_D")] = (
        -simulation_results[parameter_names.index("flux_surface_right_D")] * 1e12 / Na
    )  # D atoms/mum^2/s -> D mol/m^2/s
    simulation_results[parameter_names.index("flux_surface_right_H")] = (
        -simulation_results[parameter_names.index("flux_surface_right_H")] * 1e12 / Na
    )  # D atoms/mum^2/s -> D mol/m^2/s
    simulation_results_list.append(simulation_results)
...
for index in range(len(simulation_results_list)):
    ...
    for i in range(len(simulation_results_list[index][parameter_names.index("time")])):
        ...
        if (
            ...
            # Get pressure
            tmp_pressure_array_H2.append(
                simulation_results_list[index][
                    parameter_names.index("pressure_upstream_H2")
                ][i]
            )
            tmp_pressure_array_D2.append(
                simulation_results_list[index][
                    parameter_names.index("pressure_upstream_D2")
                ][i]
            )
            tmp_pressure_array_HD.append(
                simulation_results_list[index][
                    parameter_names.index("pressure_upstream_HD")
                ][i]
            )
            tmp_pressure_down_array_H2.append(
                simulation_results_list[index][
                    parameter_names.index("pressure_downstream_H2")
                ][i]
            )
            tmp_pressure_down_array_D2.append(
                simulation_results_list[index][
                    parameter_names.index("pressure_downstream_D2")
                ][i]
            )
            tmp_pressure_down_array_HD.append(
                simulation_results_list[index][
                    parameter_names.index("pressure_downstream_HD")
                ][i]
            )
            # Get flux
            tmp_flux_H2 = (
                (tmp_pressure_down_array_H2[-1] - pressure_enclosure1)
                * Q
                / R
                / T
                / Area
            ...
            tmp_flux_D2 = (
                (tmp_pressure_down_array_D2[-1] - pressure_enclosure1)
                * Q
                / R
                / T
                / Area
            ...
            tmp_flux_HD = (
                (tmp_pressure_down_array_HD[-1] - pressure_enclosure1)
                * Q
                / R
                / T
                / Area
...
experiment_results_list = []
experiment_file_names = [
    "experiment_mixture_H2.csv",
    "experiment_mixture_D2.csv",
    "experiment_mixture_HD.csv",
]
...
# get sum results
experiment_results_pressure_sum = (
    experiment_results_list[0][experiment_parameter_names.index("Pressure [Pa]")]
    + experiment_results_list[1][experiment_parameter_names.index("Pressure [Pa]")]
    + experiment_results_list[2][experiment_parameter_names.index("Pressure [Pa]")]
) / 3
...
time_history_plot_mixture = np.array(
    [0, 150, 150, 250, 250, 350, 350, 450, 450, 550, 550, 650, 650, 750, 750, 1000]
)  # s
pressure_history_plot_mixture_D2 = np.array(
    [
        1.8421e-4,
...
pressure_history_plot_mixture_H2 = (
    np.ones(len(pressure_history_plot_mixture_D2)) * pressure_history_H
)
plt.plot(
    time_history_plot_mixture,
    pressure_history_plot_mixture_D2,
    label=r"D$_2$",
    color="C0",
)
plt.plot(
...
ax.minorticks_on()
plt.savefig(
    "val-2e_comparison_mixture_pressure_history.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
...
ax.plot(
    pressure_array_D2[0] + pressure_array_HD[0] / 2,
    flux_array_sum[0],
    label=f"{label_name[3]} in val-2ed (TMAP8)",
    c=f"C{3}",
)
ax.plot(
    pressure_array_D2[0] + pressure_array_HD[0] / 2,
    flux_array_D2[0],
    label=f"{label_name[1]} in val-2ed (TMAP8)",
    c=f"C{1}",
)
ax.plot(
    pressure_array_D2[0] + pressure_array_HD[0] / 2,
    flux_array_H2[0],
    label=f"{label_name[0]} in val-2ed (TMAP8)",
    c=f"C{0}",
)
ax.plot(
    pressure_array_D2[0] + pressure_array_HD[0] / 2,
    flux_array_HD[0],
    label=f"{label_name[2]} in val-2ed (TMAP8)",
    c=f"C{2}",
)
...
ax.plot(
    ...
    experiment_results_flux_sum,
    signal_list[3],
    label=f"{label_name[3]} (experiment)",
    c=f"C{3}",
)
ax.plot(
    experiment_results_list[1][experiment_parameter_names.index("Pressure [Pa]")],
    experiment_results_list[1][experiment_parameter_names.index("Flux [mol/m^2/s]")],
    signal_list[1],
    label=f"{label_name[1]} (experiment)",
    c=f"C{1}",
)
ax.plot(
    experiment_results_list[0][experiment_parameter_names.index("Pressure [Pa]")],
    experiment_results_list[0][experiment_parameter_names.index("Flux [mol/m^2/s]")],
    signal_list[0],
    label=f"{label_name[0]} (experiment)",
    c=f"C{0}",
)
ax.plot(
    experiment_results_list[2][experiment_parameter_names.index("Pressure [Pa]")],
    experiment_results_list[2][experiment_parameter_names.index("Flux [mol/m^2/s]")],
    signal_list[2],
    label=f"{label_name[2]} (experiment)",
    c=f"C{2}",
)

ax.set_xlabel("Effective deuterium pressure (Pa)")
...
for index in range(len(simulation_output_list)):
    ...
    tmap_flux_for_rmspe = numerical_solution_on_experiment_input(
        experiment_input, simulation_input, simulation_output
    )
...
for i in range(len(simulation_file_names)):
    ...
    simulation_results[parameter_names.index("flux_on_left_D2")] = (
        -simulation_results[parameter_names.index("flux_on_left_D2")] * 1e12 / Na
    )  # D atoms/mum^2/s -> D mol/m^2/s
    simulation_results[parameter_names.index("flux_on_left_H2")] = (
        -simulation_results[parameter_names.index("flux_on_left_H2")] * 1e12 / Na
    )  # D atoms/mum^2/s -> D mol/m^2/s
    simulation_results[parameter_names.index("flux_on_left_HD")] = (
        -simulation_results[parameter_names.index("flux_on_left_HD")] * 1e12 / Na
    )  # D atoms/mum^2/s -> D mol/m^2/s
    simulation_results_list.append(simulation_results)
...
layer = 0
ax.plot(
    pressure_array_D2[layer] + pressure_array_HD[layer] / 2,
    flux_array_sum[layer],
    label=f"{label_name[3]} in val-2ee (TMAP8)",
    c=f"C{3}",
)
ax.plot(
    pressure_array_D2[layer] + pressure_array_HD[layer] / 2,
    flux_array_D2[layer],
    label=f"{label_name[1]} in val-2ee (TMAP8)",
    c=f"C{1}",
)
ax.plot(
    pressure_array_D2[layer] + pressure_array_HD[layer] / 2,
    flux_array_HD[layer],
    label=f"{label_name[2]} in val-2ee (TMAP8)",
    c=f"C{2}",
)
ax.plot(
    pressure_array_D2[layer] + pressure_array_HD[layer] / 2,
    flux_array_H2[layer],
    label=f"{label_name[0]} in val-2ee (TMAP8)",
    c=f"C{0}",
)
ax.plot(
    experiment_results_pressure_sum,
...
ax.minorticks_on()
plt.savefig(
    "val-2e_comparison_mixture_diffusion_recombination.png",
    bbox_inches="tight",
    dpi=300,
)
plt.close(fig)
