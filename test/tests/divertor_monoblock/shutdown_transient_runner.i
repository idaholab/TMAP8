...
[BCs]
  ...
  [C_mob_W_top_flux]
    # User defined name
    type = FunctionNeumannBC # PDE flux boundary condition: https://mooseframework.inl.gov/source/bcs/FunctionNeumannBC.html
  ...
  []
  [temp_top]
    type = FunctionNeumannBC # Applies to the variable, temperature, defined by the temp_flux_bc_func function.
...
[Functions]
  ...
  [mobile_flux_bc_func]
    type = ParsedFunction
    expression = "if(t<2e4, 7.90e-13,
                    if(t<(2e4+${peak_duration}), ${peak_value}/1.0e7*7.90e-13, 7.90e-14))"
  []
  ### Heat flux of 10MW/m^2 at steady state
  [temp_flux_bc_func]
    type = ParsedFunction
    expression = "if(t<2e4, 1.0e7,
                    if(t<(2e4+${peak_duration}), ${peak_value}, 1.0e6))"
  []
  [timestep_function]
    type = ParsedFunction
    expression = "if(t<2e4, 500,
                    if(t<(2e4+${peak_duration}+1), 0.10, 500))"
  []
[]
[Materials]
  ...
  [thermal_conductivity_W]
    ...
    expression = '${W_cond_factor}*(2.41e2 - 2.90e-1 * temperature + 2.54e-4 * temperature^2 - 1.03e-7 * temperature^3 + 1.52e-11 * temperature^4)' # ~ 173.0 [ W/m-K]
    outputs = all
  []
...
[Executioner]
  ...
  line_search = 'none'
  dtmin = 1e-6
  error_on_dtmin = False # Minimum time step for convergence, time step size is reduced upon non-convergence, but dtmin is an absolute limit. Passing this will result in an error
  nl_max_its = 36 # Maximum number of iterations for convergence
  [TimeStepper]
