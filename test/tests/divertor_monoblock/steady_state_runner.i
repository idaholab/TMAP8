...
[Functions]
  ...
  [mobile_flux_bc_function]
    ...
    expression = '${fparse C_mob_W_top_flux_val}'
  []
  [temperature_flux_bc_function]
    type = ParsedFunction
    symbol_values = 'pulse_time_function'
    symbol_names = 'pulse_time_function'
    expression = '${fparse temperature_top_val}'
  []
  [temperature_inner_func]
    type = ParsedFunction
    symbol_values = 'pulse_time_function'
    symbol_names = 'pulse_time_function'
    expression = '${fparse temperature_tube_val}'
  []
  [timestep_function]
    type = ParsedFunction
    symbol_values = 't_in_cycle'
...
[Postprocessors]
  ...
  [unused_parameters]
    ...
    expression = '${num_sectors} + ${rings_H2O} + ${rings_CuCrZr} + ${rings_Cu} + ${rings_W}
                  + ${temperature_initial} + ${temperature_coolant_max} + ${plasma_max_heat} + ${plasma_min_heat}
