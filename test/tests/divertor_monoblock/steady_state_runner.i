...
[]
[Mesh]
  # Begins the Mesh description block; the mesh spatially describes the problem, based on blocks (subdomains), meshes (geometries), nodes, etc..
  [ccmg]
    # name of the mesh
  ...
  []
  [ssbsg2]
    # Creating subdomains for W, Cu, CuCrZr and H2O and their boundaries, working inwards
...
[Problem]
  # Problem definition, i.e. the sucess criteria for the problem's time steps
...
[Variables]
  # Begin specifying variables; these are our dependent variables: what we are solving for.
  [temperature]
    # variable name
...
[AuxVariables]
  # Define auxillary variables; these are NOT solved by the problem's defining PDE's (defined by the Kernels later). AuxKernels will be used to define the AuxVariables as functions of the known variables (solved for by the Kernels)
  [flux_y]
    order = FIRST
...
[Kernels]
  # Begin Kernel Section. Kernels are a piece of physics, representing one or more operators in the weak form of a PDE, with all terms on the left side = 0, their sum is the residual.
  ############################## Kernels for W (block = 4)
  [diff_W]
    # Kernel name (user-specified)
...
[AuxKernels]
  # Auxillary Kernels used to calculate AuxVariables from the solved variables
  ############################## AuxKernels for W (block = 4)
  [Scaled_mobile_W]
    # user-specificed name
...
[InterfaceKernels]
  # Specifying specific physis/equations for material interfaces.
  [tied_4to3]
    # Interface name (user-specified)
...
[NodalKernels]
  # Nodal Kernels are used to solve equations that specifically belong on a node.
...
[]
[Functions]
  # Specific physical relationship, allowed to be used by multiple objects
  [timestep_function]
    type = ParsedFunction
...
[]
[Materials]
  # Material properties and relationships to apply to subdomains.
...
[Postprocessors]
  ...
  []
  [F_permeation]
    # Uses conversion factor, not really flux
  ...
  []
  [temperature_top]
    # Reports variable value at a specific point (x,y,z)
    type = PointValue
    variable = temperature
    point = '0 14.0e-3 0'
    execute_on = 'MULTIAPP_FIXED_POINT_END FINAL'
