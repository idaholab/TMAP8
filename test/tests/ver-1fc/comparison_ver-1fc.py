...
if "/tmap8/doc/" in script_folder.lower():  # if in documentation folder
    csv_folder = (
        "../../../../test/tests/ver-1fc/gold/ver-1fc_vector_postproc_line_0063.csv"
    )
...
if "/tmap8/doc/" in script_folder.lower():  # if in documentation folder
    csv_folder = (
        "../../../../test/tests/ver-1fc/gold/ver-1fc_vector_postproc_line_0032.csv"
    )
else:  # if in test folder
...
if "/tmap8/doc/" in script_folder.lower():  # if in documentation folder
    csv_folder = (
        "../../../../test/tests/ver-1fc/ver-1fc_abaqus_TMAP7_results_over_distance.csv"
    )
else:  # if in test folder
...
abaqus_sol_transient_temperature_1 = abaqus_sol_transient[
    "abaqus_or_TMAP7_temperature_transient_1"
]
abaqus_sol_transient_temperature_2 = abaqus_sol_transient[
    "abaqus_or_TMAP7_temperature_transient_2"
]

if "/tmap8/doc/" in script_folder.lower():  # if in documentation folder
    csv_folder = (
        "../../../../test/tests/ver-1fc/ver-1fc_abaqus_TMAP7_results_over_time.csv"
    )
else:  # if in test folder
    csv_folder = "./ver-1fc_abaqus_TMAP7_results_over_time.csv"
abaqus_sol_transient = pd.read_csv(csv_folder)
abaqus_sol_transient_t = abaqus_sol_transient["time"]
abaqus_sol_transient_temperature_at_x = abaqus_sol_transient[
    "abaqus_temperature_transient"
]
tmap7_sol_transient_temperature_at_x = abaqus_sol_transient[
    "TMAP7_temperature_transient"
]
...
analytical_sol_steady_state_x = get_analytical_solution_steady_state(
    tmap8_sol_steady_state_x
)
...
ax.plot(
    tmap8_sol_steady_state_x,
    tmap8_sol_steady_state_temperature,
    label=r"TMAP8",
    c="tab:gray",
)
ax.plot(
    tmap8_sol_steady_state_x,
    analytical_sol_steady_state_x,
    label=r"Analytical",
    c="k",
    linestyle="--",
)
...
RMSE = np.linalg.norm(
    tmap8_sol_steady_state_temperature - analytical_sol_steady_state_x
)
err_percent = RMSE * 100 / np.mean(analytical_sol_steady_state_x)
ax.text(0.55, 400, "RMSPE = %.2f " % err_percent + "% \n", fontweight="bold")
plt.savefig(
    "ver-1fc_comparison_temperature_steady_state.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
...
ax = fig.add_subplot(gs[0])
ax.plot(
    tmap8_sol_transient_x, tmap8_sol_transient_temperature, label=r"TMAP8", c="tab:gray"
)
ax.plot(
    abaqus_sol_transient_x,
    abaqus_sol_transient_temperature_1,
    "b^",
    label=r"ABAQUS or TMAP7 (1)",
    mfc="none",
)
ax.plot(
    abaqus_sol_transient_x,
    abaqus_sol_transient_temperature_2,
    "ro",
    label=r"ABAQUS or TMAP7 (2)",
    mfc="none",
)
ax.set_xlabel("Distance (m)")
ax.set_ylabel(r"Temperature (K)")
...
ax.minorticks_on()
plt.savefig(
    "ver-1fc_comparison_temperature_transient_t150.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
...
ax = fig.add_subplot(gs[0])
ax.plot(
    tmap8_sol_transient_t,
    tmap8_sol_transient_temperature_at_x,
    label=r"TMAP8",
    c="tab:gray",
)
ax.plot(
    abaqus_sol_transient_t,
    abaqus_sol_transient_temperature_at_x,
    "b^",
    label=r"ABAQUS",
    mfc="none",
)
ax.plot(
    abaqus_sol_transient_t,
    tmap7_sol_transient_temperature_at_x,
    "ro",
    label=r"TMAP7",
    mfc="none",
)
...
ax.minorticks_on()
plt.savefig(
    "ver-1fc_comparison_temperature_transient_x0.09.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
