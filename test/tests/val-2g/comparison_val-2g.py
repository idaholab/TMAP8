...
simulation_results2[IDX_FLUX_T2_DRY] = (
    simulation_results2[IDX_FLUX_T2_DRY] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s
simulation_results2[IDX_FLUX_T2O_DRY] = (
    simulation_results2[IDX_FLUX_T2O_DRY] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s
simulation_results2[IDX_FLUX_T2O_WET] = (
    simulation_results2[IDX_FLUX_T2O_WET] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s
simulation_results2[IDX_FLUX_T2_WET] = (
    simulation_results2[IDX_FLUX_T2_WET] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s
...
simulation_results3[IDX_FLUX_T2_DRY] = (
    simulation_results3[IDX_FLUX_T2_DRY] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s
simulation_results3[IDX_FLUX_T2O_DRY] = (
    simulation_results3[IDX_FLUX_T2O_DRY] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s
simulation_results3[IDX_FLUX_T2O_WET] = (
    simulation_results3[IDX_FLUX_T2O_WET] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s
simulation_results3[IDX_FLUX_T2_WET] = (
    simulation_results3[IDX_FLUX_T2_WET] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s

file_name = "./val-2g_trapping_calibrated.csv"
simulation_results4 = read_csv_from_TMAP8(file_name, parameter_names)  # read csv file
simulation_results4[IDX_FLUX_T2_DRY] = (
    simulation_results4[IDX_FLUX_T2_DRY] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s
simulation_results4[IDX_FLUX_T2O_DRY] = (
    simulation_results4[IDX_FLUX_T2O_DRY] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s
simulation_results4[IDX_FLUX_T2O_WET] = (
    simulation_results4[IDX_FLUX_T2O_WET] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s
simulation_results4[IDX_FLUX_T2_WET] = (
    simulation_results4[IDX_FLUX_T2_WET] * 1e12 * 2
)  # atoms/mum^2/s -> atoms/m^2/s
...
experiment_results1 = read_csv_from_TMAP8(
    file_name, experiment_parameter_names
)  # read csv file
file_name = "./BZY_873K_D2_exposed_D2O_flux.csv"  # D2O under dry
experiment_results2 = read_csv_from_TMAP8(
    file_name, experiment_parameter_names
)  # read csv file
file_name = "./BZY_873K_D2O_exposed_D2_flux.csv"  # D2 under wet
experiment_results3 = read_csv_from_TMAP8(
    file_name, experiment_parameter_names
)  # read csv file
file_name = "./BZY_873K_D2O_exposed_D2O_flux.csv"  # D2O under wet
experiment_results4 = read_csv_from_TMAP8(
    file_name, experiment_parameter_names
)  # read csv file

# Experiment results
experiment_input = (
    experiment_results1[experiment_parameter_names.index("Time")]
    - dissolve_duration
    - cooldown_duration
) * temperature_rate + temperature_low
experiment_output = experiment_results1[experiment_parameter_names.index("Flux")]
experiment2_input = (
    experiment_results2[experiment_parameter_names.index("Time")]
    - dissolve_duration
    - cooldown_duration
) * temperature_rate + temperature_low
...
experiment3_input = (
    experiment_results3[experiment_parameter_names.index("Time")]
    - dissolve_duration
    - cooldown_duration
) * temperature_rate + temperature_low
experiment3_output = experiment_results3[experiment_parameter_names.index("Flux")]
experiment4_input = (
    experiment_results4[experiment_parameter_names.index("Time")]
    - dissolve_duration
    - cooldown_duration
) * temperature_rate + temperature_low
...
ax1.plot(
    simulation_results2[parameter_names.index("time")],
    simulation_results2[parameter_names.index("pressure_T2_average")],
    "-",
    label=rf"D$_2$ pressure",
    c="C0",
)
ax1.plot(
    simulation_results2[parameter_names.index("time")],
    simulation_results2[parameter_names.index("pressure_T2O_average")],
    "--",
    label=rf"D$_2$O pressure",
    c="C0",
)
ax1.set_xlabel("Time (s)", fontsize=14)
ax1.set_ylabel("Pressure (Pa)", color="C0", fontsize=14)
ax1.set_yscale("log")
...
ax2.plot(
    simulation_results2[parameter_names.index("time")],
    simulation_results2[parameter_names.index("temperature_average")],
    label=rf"temperature",
    c="C1",
)
ax2.set_ylabel("Temperature (K)", color="C1", fontsize=14)
...
ax1.plot(
    ...
    experiment_output,
    "s",
    label=rf"experiment: D from D$_2$",
    c="C0",
    markersize=3,
)
ax1.plot(
    simulation_results2[IDX_TEMP][chosen_matrix2],
    simulation_results2[IDX_FLUX_T2_DRY][chosen_matrix2] * A / N_A,
    label=rf"no trapping: D from D$_2$",
    c="C0",
)
ax1.plot(
    simulation_results2[IDX_TEMP][chosen_matrix2],
    simulation_results2[IDX_FLUX_T2O_DRY][chosen_matrix2] * A / N_A,
    label=rf"no trapping: D from D$_2$O",
    c="C1",
)
ax1.plot(
    experiment2_input,
    experiment2_output,
    ".",
    label=rf"experiment: D from D$_2$O",
    c="C1",
)
ax1.set_xlabel("Temperature (K)", fontsize=14)
ax1.set_ylabel("Deuterium flux (mol/s)", fontsize=14)
ax1.grid(visible=True, which="major", color="0.65", linestyle="--", alpha=0.3)
ax1.minorticks_on()
...
flux_trapping = np.interp(
    experiment_input,
    simulation_result2_temperature_order,
    simulation_result2_flux_order,
)
RMSPE = (
    np.sqrt(np.mean((flux_trapping * A / N_A - experiment_output) ** 2))
    / np.mean(experiment_output)
    * 100
)
ax1.text(
    380, 1.0e-11, "RMSPE = %.2f " % RMSPE + "%", fontweight="bold", c="C0", fontsize=11
)
simulation_result2_temperature = simulation_results2[IDX_TEMP][chosen_matrix2]
...
RMSPE = (
    np.sqrt(np.mean((flux_trapping * A / N_A - experiment2_output) ** 2))
    / np.mean(experiment2_output)
    * 100
)
ax1.text(
    380, 0.35e-11, "RMSPE = %.2f " % RMSPE + "%", fontweight="bold", c="C1", fontsize=11
)
plt.legend(fontsize=12)
plt.savefig(
    "./val-2g_dry_no_trapping_flux_comparison.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
...
ax1.plot(
    simulation_results2[IDX_TEMP][chosen_matrix2],
    simulation_results2[IDX_FLUX_T2O_WET][chosen_matrix2] * A / N_A,
    label=rf"no trapping: D from D$_2$O",
    c="C1",
)
ax1.plot(
...
ax1.plot(
    simulation_results2[IDX_TEMP][chosen_matrix2],
    simulation_results2[IDX_FLUX_T2_WET][chosen_matrix2] * A / N_A,
    label=rf"no trapping: D from D$_2$",
    c="C0",
)
ax1.set_xlabel("Temperature (K)", fontsize=14)
...
RMSPE = (
    np.sqrt(np.mean((flux_trapping * A / N_A - experiment3_output) ** 2))
    / np.mean(experiment3_output)
    * 100
)
ax1.text(
    380, 0.5e-9, "RMSPE = %.2f " % RMSPE + "%", fontweight="bold", c="C0", fontsize=11
)
simulation_result2_temperature = simulation_results2[IDX_TEMP][chosen_matrix2]
...
RMSPE = (
    np.sqrt(np.mean((flux_trapping * A / N_A - experiment4_output) ** 2))
    / np.mean(experiment4_output)
    * 100
)
ax1.text(
    380, 1.45e-9, "RMSPE = %.2f " % RMSPE + "%", fontweight="bold", c="C1", fontsize=11
)
plt.legend(fontsize=12)
plt.savefig(
    "./val-2g_wet_no_trapping_flux_comparison.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
...
ax1.plot(
    simulation_results3[IDX_TEMP][chosen_matrix3],
    simulation_results3[IDX_FLUX_T2_DRY][chosen_matrix3] * A / N_A,
    label=rf"trapping: D from D$_2$",
    c="C0",
)
ax1.plot(
    simulation_results3[IDX_TEMP][chosen_matrix3],
    simulation_results3[IDX_FLUX_T2O_DRY][chosen_matrix3] * A / N_A,
    label=rf"trapping: D from D$_2$O",
    c="C1",
)
ax1.plot(
    experiment2_input,
...
ax1.text(
    350, 1.0e-11, "RMSPE = %.2f " % RMSPE + "%", fontweight="bold", c="C0", fontsize=11
)
simulation_result3_temperature = simulation_results3[IDX_TEMP][chosen_matrix3]
...
ax1.text(
    1100, 1.0e-11, "RMSPE = %.2f " % RMSPE + "%", fontweight="bold", c="C1", fontsize=11
)
plt.legend(fontsize=12)
...
ax1.plot(
    simulation_results3[IDX_TEMP][chosen_matrix3],
    simulation_results3[IDX_FLUX_T2O_WET][chosen_matrix3] * A / N_A,
    label=rf"trapping: D from D$_2$O",
    c="C1",
)
ax1.plot(
...
ax1.plot(
    simulation_results3[IDX_TEMP][chosen_matrix3],
    simulation_results3[IDX_FLUX_T2_WET][chosen_matrix3] * A / N_A,
    label=rf"trapping: D from D$_2$",
    c="C0",
)
ax1.set_xlabel("Temperature (K)", fontsize=14)
...
ax1.text(
    350, 1.0e-9, "RMSPE = %.2f " % RMSPE + "%", fontweight="bold", c="C0", fontsize=11
)
simulation_result3_temperature = simulation_results3[IDX_TEMP][chosen_matrix3]
...
ax1.text(
    1100, 1.0e-9, "RMSPE = %.2f " % RMSPE + "%", fontweight="bold", c="C1", fontsize=11
)
plt.legend(fontsize=12)
...
ax1.plot(
    ...
    experiment_output,
    "s",
    label="experiment: D from D2",
    c="C0",
    markersize=3,
)
ax1.plot(
    simulation_results4[IDX_TEMP][chosen_matrix4],
    simulation_results4[IDX_FLUX_T2_DRY][chosen_matrix4] * A / N_A,
    label=rf"trapping: D from D$_2$",
    c="C0",
)
ax1.plot(
    simulation_results4[IDX_TEMP][chosen_matrix4],
    simulation_results4[IDX_FLUX_T2O_DRY][chosen_matrix4] * A / N_A,
    label=rf"trapping: D from D$_2$O",
    c="C1",
)
ax1.plot(
    experiment2_input, experiment2_output, ".", label="experiment: D from D2O", c="C1"
)
ax1.set_xlabel("Temperature (K)", fontsize=14)
ax1.set_ylabel("Deuterium flux (mol/s)", fontsize=14)
ax1.set_xlim([300, 1400])
...
plt.legend(fontsize=12)
plt.savefig(
    "./val-2g_dry_trapping_flux_calibrated_comparison.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
...
ax1.plot(
    simulation_results4[IDX_TEMP][chosen_matrix4],
    simulation_results4[IDX_FLUX_T2O_WET][chosen_matrix4] * A / N_A,
    label=rf"trapping: D from D$_2$O",
    c="C1",
)
ax1.plot(
    experiment4_input, experiment4_output, ".", label="experiment: D from D2O", c="C1"
)
ax1.plot(
    experiment3_input,
    experiment3_output,
...
ax1.plot(
    simulation_results4[IDX_TEMP][chosen_matrix4],
    simulation_results4[IDX_FLUX_T2_WET][chosen_matrix4] * A / N_A,
    label=rf"trapping: D from D$_2$",
    c="C0",
)
ax1.set_xlabel("Temperature (K)", fontsize=14)
...
plt.legend(fontsize=12)
plt.savefig(
    "./val-2g_wet_trapping_flux_calibrated_comparison.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
...
for ii in np.arange(1, num_iter + 1, 1):
    ...
    obj_values[ii - 1, :] = np.array(
        data["time_steps"][ii]["PSS_reporter"]["output_required"]
    )
...
plt.savefig(
    "./val-2g_trapping_optimization_PSS_iterations", bbox_inches="tight", dpi=300
)
plt.close()
...
for i in range(dim):
    ...
    norm_optimized_input = (
        input_optimized[i] - corresponding_ave[i]
    ) / corresponding_std[i]
    ax1.plot(
        [norm_optimized_input, norm_optimized_input],
        y_limit,
        "--",
        color=colors[i],
        label=label_names[i],
...
plt.savefig(
    "./val-2g_trapping_optimization_inputs_PSS.png", bbox_inches="tight", dpi=300
)
plt.close(fig)
