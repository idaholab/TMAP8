...
def get_analytical_solution(t_vect):
    ...
    concentration_B = (
        k_1
        * concentration_A_0
        * (np.exp(-k_1 * t_vect) - np.exp(-k_2 * t_vect))
        / (k_2 - k_1)
    )
...
ax.plot(
    tmap8_sol_time,
    tmap8_sol_concentration_C,
    label=r"$c_C$ TMAP8",
    c="tab:green",
    alpha=alpha,
)
concentration_A, concentration_B, concentration_C = get_analytical_solution(
    tmap8_sol_time
)
ax.plot(
    tmap8_sol_time, concentration_A, label=r"$c_A$ Analytical", c="m", linestyle="--"
)
ax.plot(
    tmap8_sol_time, concentration_B, label=r"$c_B$ Analytical", c="b", linestyle="--"
)
ax.plot(
    tmap8_sol_time, concentration_C, label=r"$c_C$ Analytical", c="g", linestyle="--"
)
...
RMSE_A = np.linalg.norm(tmap8_sol_concentration_A - concentration_A)
err_percent_A = RMSE_A * 100 / np.mean(concentration_A)
ax.text(
    50,
    2.05e14,
    "(A) RMSPE = %.2f " % err_percent_A + "%",
    fontweight="bold",
    color="tab:pink",
)
RMSE_B = np.linalg.norm(tmap8_sol_concentration_B - concentration_B)
err_percent_B = RMSE_B * 100 / np.mean(concentration_B)
ax.text(
    80,
    1.70e14,
    "(B) RMSPE = %.2f " % err_percent_B + "%",
    fontweight="bold",
    color="tab:blue",
)
