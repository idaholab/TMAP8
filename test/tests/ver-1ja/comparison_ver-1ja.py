...
analytical_tritium = tritium_concentration_initial * np.exp(
    -decay_rate_constant * tmap8_time
)
analytical_helium = tritium_concentration_initial * (
...
ax.plot(
    tmap8_time_years, tmap8_tritium, label=r"$C_T$ - TMAP8", c="tab:blue", alpha=0.5
)
ax.plot(
    tmap8_time_years,
    analytical_tritium,
    label=r"$C_T$ - Analytical",
    c="b",
    linestyle="--",
)
ax.plot(
    tmap8_time_years, tmap8_helium, label=r"$C_{He}$ - TMAP8", c="tab:red", alpha=0.5
)
ax.plot(
    tmap8_time_years,
    analytical_helium,
    label=r"$C_{He}$ - Analytical",
    c="r",
    linestyle="--",
)
...
ax.text(
    65,
    13.8e4,
    "$C_{He}$ RMSPE = %.2f " % err_percent_CHe + "%",
    fontweight="bold",
    color="tab:red",
)
