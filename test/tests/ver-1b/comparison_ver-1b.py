...
if "/tmap8/doc/" in script_folder.lower():  # if in documentation folder
    csv_folder = (
        "../../../../test/tests/ver-1b/gold/ver-1b_vector_postproc_line_0250.csv"
    )
else:  # if in test folder
...
ax.plot(
    analytical_distance, analytical_conc, label=r"Analytical", c="k", linestyle="--"
)
...
analytical_flux = (
    C_o
    * np.sqrt(D / (np.pi * analytical_time))
    * np.exp(x / (2 * np.sqrt(D * analytical_time)))
)
ax.plot(tmap_time[1:], tmap_flux[1:], label=r"TMAP8", c="tab:gray")
ax.plot(
    analytical_time[1:], analytical_flux[1:], label=r"Analytical", c="k", linestyle="--"
)
