...
ADMatCoupledDefectAnnihilation::validParams()
{
  ...
  params.addClassDescription("Kernel to add K*v*(c0-c), where K=annihilation rate, u=variable, v=coupled variable");
  params.addParam<MaterialPropertyName>("eq_concentration", "u_0", "The equilibrium concentration of the variable used with the kernel");
  params.addParam<MaterialPropertyName>("annihilation_rate", "K", "The annihilation rate used with the kernel");
  params.addParam<Real>("coeff", 1., "A coefficient for multiplying the The annihilation rate. It can be used for sensitivity analysis.");
  return params;
}
